...
public class NearFeatureFinder {
    ...
    // mmichaud : change several methods to make it compatible with
    // CoverageCleaner
    //public NearFeatureFinder() {}
    IndexedFeatureCollection ifc;
    
    // mmichaud added to be consistent with CoverageCleaner
    public NearFeatureFinder(FeatureCollection fc) {
        ifc = new IndexedFeatureCollection(fc);
    }
     ...
     */
    public List findNearFeatures(Feature f, double maxDist) {
        Geometry g = f.getGeometry();
        Envelope envelope = g.getEnvelopeInternal();
        Envelope searchEnvelope = EnvelopeUtil.expand(envelope, maxDist);
        List candidates = ifc.query(searchEnvelope);
        List nearFeatures = new ArrayList<Feature>();
        for (Iterator it = candidates.iterator() ; it.hasNext() ; ) {
            Feature candidate = (Feature)it.next();
            //if (candidate.getGeometry().distance(g) <= maxDist) {
                nearFeatures.add(candidate);
            //}
        }
        return nearFeatures;
    }
     ...
     */
     /*
    public List findNearFeatures(Feature f, FeatureCollection targetFC, double maxDist) {
