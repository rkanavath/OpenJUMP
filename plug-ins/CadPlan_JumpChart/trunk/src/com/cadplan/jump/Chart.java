...
public class Chart
{
    ...
    public Chart(PlugInContext context, I18NPlug iPlug)
    {
        ...
        if(!ChartParams.cancelled)
        {
            ...
            {
            	//System.out.println("Att i="+i+ "  value="+ chartValues[i].toString());
            ...
            Map<Integer,BasicStyle> themeMap = new HashMap<Integer,BasicStyle>(); ;
            ...
            for(int i=0; i < numElements; i++)
            {
                ...
                int alpha = 128;
                Color color = new Color(Color.HSBtoRGB((float)i/(float)numElements, 1.0f,1.0f), true);
            ...
            ColorThemingStyle themingStyle = new ColorThemingStyle("Index", themeMap, labelMap, basicStyle);
        ...
        double maxSize = 0.0;
         ...
         switch (ChartParams.chartType)
         {
             case ChartParams.PIE_EAST:
                        PieChart pieChart = new PieChart(chartAttributes, chartValues, screenScale, 0.0);
                        pieChart.addFeatures(dataset);
                        maxValue = pieChart.getMaxValue();
                        maxSize = pieChart.getMaxSize();
                        break;
             case ChartParams.PIE_NORTH:
                        pieChart = new PieChart(chartAttributes, chartValues, screenScale, Math.PI/2.0);
                        pieChart.addFeatures(dataset);
                        maxValue = pieChart.getMaxValue();
                        maxSize = pieChart.getMaxSize();
                        break;
             case ChartParams.BAR:
                        BarChart barChart = new BarChart(chartAttributes, chartValues, screenScale);
             ...
             case ChartParams.COLUMN:
                         ColumnChart columnChart = new ColumnChart(chartAttributes, chartValues, screenScale);
                         columnChart.addFeatures(dataset);
                         maxValue = columnChart.getMaxValue();
                         maxSize = columnChart.getMaxSize();
                         break;
             case ChartParams.LABELS:
		            	 LabelsChart labelsChart = new LabelsChart(chartAttributes, chartValues, screenScale);
		                 labelsChart.addFeatures(dataset);
		                 
            	         break;
         }
         if(ChartParams.includeLegend && ChartParams.chartType != ChartParams.LABELS)   createLegend(dataset);
         if(ChartParams.showScale && !ChartParams.uniformSize)
         {
               ...
               if(debug) System.out.println("maxValue="+maxValue+"  maxSize="+maxSize+"  scale="+scale);
    ...
    private void createScale(int type, FeatureCollection dataset, double interval, double scale)
    {
        FeatureSchema featureSchema = new FeatureSchema();
        featureSchema.addAttribute("Geometry", AttributeType.GEOMETRY);
        featureSchema.addAttribute("dValue", AttributeType.DOUBLE);
        ...
        double x0 = context.getLayerViewPanel().getViewport().getEnvelopeInModelCoordinates().getMinX();
        double y0 = context.getLayerViewPanel().getViewport().getEnvelopeInModelCoordinates().getMinY();
        ...
        switch (type)
        {
            ...
            case ChartParams.PIE_NORTH:
                    ...
                    for(int i=4; i >= 0; i--)
                    {
                        ...
                        for(int k=0; k < 32; k++)
                        {
                            double xp = x + rada*Math.cos((double)k*2.0*Math.PI/(double)nsides);
                            double yp = y + rada + rada*Math.sin((double)k*2.0*Math.PI/(double)nsides);

                            circlePoints[k] = new Coordinate(xp,yp);
                        }
                        circlePoints[nsides] = circlePoints[0];
                        //Geometry geometry = new GeometryFactory().createLineString(circlePoints);
                        LinearRing lr = new GeometryFactory().createLinearRing(circlePoints);
                        ...
                        linePoints[1] = new Coordinate(x+scale*5.2*interval, y+rada*2.0);
                       ...
                       Geometry linegeometry = new GeometryFactory().createLineString(linePoints);
                        ...
                        if(debug) System.out.println("Geometry: "+linegeometry);
                        ...
                        //pointPoints[0] = new Coordinate(x+scale*8.0*interval, y+rad*2.0);
                        //Geometry pointgeometry = new GeometryFactory().createMultiPoint(pointPoints);
                        
                        Coordinate [] pointPoints = new Coordinate[2];
                        pointPoints[0] = new Coordinate(x+scale*5.2*interval,y+rada*2.0);
                        pointPoints[1] = new Coordinate(x+scale*8.0*interval, y+rada*2.0);
                        //Geometry pointgeometry = new GeometryFactory().createLineString(pointPoints);
                        Coordinate position = new Coordinate(x+scale*6.2*interval,y+rada*2.0);
                        Geometry pointgeometry = new GeometryFactory().createPoint(position);
            ...
            case ChartParams.COLUMN:
                    ...
                    for(int i=4; i >= 0; i--)
                    {
                        ...
                        linePoints[1] = new Coordinate(x+2.0*width, yp+interval*scale);
                        ...
                        //Coordinate [] pointPoints = new Coordinate[1];
                        //pointPoints[0] = new Coordinate(x+4*width, yp+interval*scale);                      
                        //Geometry pointgeometry = new GeometryFactory().createMultiPoint(pointPoints);
                        
                        Coordinate [] pointPoints = new Coordinate[2];
                        pointPoints[0] = new Coordinate(x+2.0*width,yp+interval*scale);
                        pointPoints[1] = new Coordinate(x+6.0*width, yp+interval*scale);
                        Geometry pointgeometry = new GeometryFactory().createLineString(pointPoints);
    ...
    private void createLegend(FeatureCollection dataset)
    {
        ...
        Geometry geometry = new GeometryFactory().createLineString(borderArray);
        Feature feature = new BasicFeature(featureSchema);
        feature.setGeometry(geometry);
        ...
        if(!ChartParams.uniformSize)
        {
            ...
            Geometry labelGeometry = new GeometryFactory().createPolygon(lr, null);

            //Geometry labelGeometry = new GeometryFactory().createLineString(labelArray);
            ...
            labelFeature.setAttribute("Name", iPlug.get("JumpChart.Legend.ChartType"));
