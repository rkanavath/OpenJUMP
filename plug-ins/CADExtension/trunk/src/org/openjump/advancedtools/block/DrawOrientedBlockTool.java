...
public class DrawOrientedBlockTool extends ConstrainedNClickTool {
    ...
    public int getDegreesAzimuth() {
        this.angle = new Double(
                new Long(
                        Math.round(metrics.azimuth(getCoordinates()) * 10000.0D))
                        .doubleValue() / 10000.0D).doubleValue();
        return (int) this.angle;
    }

    @SuppressWarnings("static-access")
    public int getDegreesAngle() {
        this.angle = new Double(new Long(Math.round(metrics
                .angle(getCoordinates()) * 10000.0D)).doubleValue() / 10000.0D)
                .doubleValue();
        return (int) this.angle;
    }
    ...
    private void calculateShape_0(Coordinate middlePoint, LayerViewPanel panel)
    ...
    protected Shape getShape() throws NoninvertibleTransformException {
        ...
        try {
            if (getCoordinates().size() == 0) {
                ...
                path.lineTo((float) secondPoint.getX(),
                ...
                path.moveTo((float) firstPoint.getX(),
                        (float) firstPoint.getY());
                path.lineTo((float) thirdPoint.getX(),
