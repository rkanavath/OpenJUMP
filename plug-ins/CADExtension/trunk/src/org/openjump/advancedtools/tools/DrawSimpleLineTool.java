...
public class DrawSimpleLineTool extends NClickTool {
    ...
    public final static String NAME = I18NPlug
            .getI18N("org.openjump.core.ui.tools.DrawSimpleLine.Draw-simple-line");
    public final static String DESCRIPTION = I18NPlug
            .getI18N("org.openjump.core.ui.tools.DrawSimpleLine.description");
    ...
    protected SimpleLineDialog sld = null;
     ...
     * @param c2
     */
    public DrawSimpleLineTool(int n, SimpleLineDialog sld, Coordinate c1,
            Coordinate c2) {
        super(n);
        coordinateA = c1;
        coordinateB = c2;
    ...
    protected void gestureFinished() throws Exception {
        ...
        if (!sld.isSegundoPuntoCapturado()) {
            if (!sld.relativo) {
                ...
                coordinateB = new Coordinate(sld.x2 + coordinateA.x, sld.y2
                        + coordinateA.y);
            }
        } else {
            coordinateB = (Coordinate) getCoordinates().get(
                    getCoordinates().size() - 1);
            if (sld.longitud != -1) {
                double angulo = -EditUtils.getAngle(coordinateA.x,
                        coordinateA.y, coordinateB.x, coordinateB.y);
                coordinateB = new Coordinate(coordinateA.x
                        + (Math.sin(angulo) * sld.longitud), coordinateA.y
                        + (Math.cos(angulo) * sld.longitud));
            }
    ...
    public Feature createFeature() {
        ...
        return FeatureUtil.toFeature(createGeometry(), editableLayer
    ...
    public void save() throws Exception {
        ...
        final List<Feature> featsToAdd = new ArrayList<Feature>();
        featsToAdd.add(createFeature());

        WorkbenchUtils.executeUndoableAddNewFeats(NAME, getPanel()
                .getSelectionManager(), editableLayer, featsToAdd);
