...
public class OffsetBuilder {
     ...
     */
    public ArrayList<Coordinate> getLineCurve(Coordinate[] inputPts,
            double distance) {
    ...
    private void addLastSegmentCW() {
        ...
        if ((lastOutsideTurn != null && lastOutsideTurn)
                || vertexList.size() == 0) {
    ...
    private void addCollinear(boolean addStartPoint) {
         ...
         * In this case the point can be ignored, since the offset lines will
         * also be parallel.
         */
        if (numInt >= 2) {
            /**
             * segments are collinear but reversing. Add an "end-cap" fillet all
             * the way around to other direction This case should ONLY happen
             * for LineStrings, so the orientation is always CW. (Polygons can
             * never have two consecutive segments which are parallel but
             * reversed, because that would be a self intersection.
             * 
             */
            // TODO check this case
            addFillet(s1, offset0.p1, offset1.p0, CGAlgorithms.CLOCKWISE,
                    distance);
        }
    }

    /**
     * Add points for a circular fillet around a reflex corner. Adds the start
     * and end points
     * 
     * @param p
     *            base point of curve
     ...
     * @param b
     */
    private void addCornerPoint(LineSegment offset0, LineSegment offset1,
            boolean b) {
     ...
     * start and end point for the fillet are not added - the caller must add
     * them if required.
     * 
     * @param direction
