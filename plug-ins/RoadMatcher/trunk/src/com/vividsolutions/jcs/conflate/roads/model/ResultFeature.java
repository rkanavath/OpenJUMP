...
public class ResultFeature implements Feature {
    ...
    public Feature clone(boolean deep, boolean copyPK) {
        BasicFeature clone = new BasicFeature(getSchema());
        for (int i = 0; i < getSchema().getAttributeCount(); i++) {
            if (getSchema().getAttributeType(i) == AttributeType.GEOMETRY) {
                clone.setAttribute(i, deep ? getGeometry().clone() : getGeometry());
            } else if (getSchema().getExternalPrimaryKeyIndex() == i) {
                if (copyPK) clone.setAttribute(i, getAttribute(i));
            } else {
