...
import ca.ucalgary.engg.moveantools.util.ScaledLineKernelDensityForMovementTracksAlgorithm;
 ...
 *
 **/
public class ScaledLineKernelDensityForMovementTracksPlugIn extends AbstractThreadedUiPlugIn{
    ...
    private String sSidebar ="Creates a density raster based on line-based Kernel-Density - with modifications for " +
    		...
    		" - to avoid summation on ends and a scaling so as to create a bone shape of the single line segment. " +
    		"Note, only the Sextante-Biweight kernel is implemented so far.";   
    ...
    private final String sScaleWithSqrt = "Kernel scaling function from [1..0.707] : otherwise [1..0.5]";
    ...
    private boolean scaleWithSqrt = true;
    ...
    public void initialize(PlugInContext context) throws Exception {
        	...
        	context.getFeatureInstaller().addMainMenuItem(
                    ...
                    this,
                    new JMenuItem("Scaled Line-based Kernel Density for Movement Points...", null),
	...
	private void initDialog(PlugInContext context) {
        ...
        dialog = new MultiInputDialog(context.getWorkbenchFrame(), "Scaled Line-KD for Movement", true);
	    ...
	    dialog.addCheckBox(sScaleWithSqrt, this.scaleWithSqrt);
	    dialog.addSeparator();
    ...
    private void getDialogValues(MultiInputDialog dialog) {
        ...
        this.scaleWithSqrt = dialog.getBoolean(this.sScaleWithSqrt);
      }
	...
	private RasterImageLayer calcLKDensityGrid(FeatureCollection singleLines,
			double bandwidth, double cellSize, boolean rasterizeLFirst, boolean scaleWithSQRT, PlugInContext context, TaskMonitor monitor) 
			throws GeoAlgorithmExecutionException {
		...
		ScaledLineKernelDensityForMovementTracksAlgorithm alg = new ScaledLineKernelDensityForMovementTracksAlgorithm();
		ParametersSet params = alg.getParameters();
		boolean worked = params.getParameter(ScaledLineKernelDensityForMovementTracksAlgorithm.LINELAYER).setParameterValue(ojlineLayer);
		if(worked){
			params.getParameter(ScaledLineKernelDensityForMovementTracksAlgorithm.IDFIELD).setParameterValue(ojlineLayer.getFieldIndexByName("bufferProcID"));
			params.getParameter(ScaledLineKernelDensityForMovementTracksAlgorithm.BANDWIDTH).setParameterValue(new Double(bandwidth));
			params.getParameter(ScaledLineKernelDensityForMovementTracksAlgorithm.SCALEWITHSQRT).setParameterValue(new Boolean(scaleWithSQRT));
			//params.getParameter(ScaledLineKernelDensityForMovementTracksAlgorithm.RASTERIZEFIRST).setParameterValue(new Boolean(rasterizeLFirst));
			...
			Output raster = outputs.getOutput(ScaledLineKernelDensityForMovementTracksAlgorithm.RESULT);		
			if(monitor != null){
