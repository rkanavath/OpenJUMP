...
public class DeeJUMPWorkbench extends JUMPWorkbench{
	...
	public static ImageIcon splashImage() {
		...
		if (splashImage == null) {
//			splashImage = IconLoader.icon("splash.png");					//"gp-desktop-splash.png"
			splashImage = new ImageIcon(DeeJUMPWorkbench.class.getResource("deejump_splash.png" ));
		}
	...
	private static final ImageIcon APP_ICON = 
	    new ImageIcon(DeeJUMPWorkbench.class.getResource("latlon_icon.png" ));
...
*/
	 ...
	 */
	public DeeJUMPWorkbench(String title, String[] args, ImageIcon icon,
			final JWindow s, TaskMonitor monitor) throws Exception {
	    super( "deeJUMP" , args, icon, s, monitor);
	    this.getFrame().getToolBar().setRollover( true );
	...
	public static void main(String[] args) {
		try {
			...
			main(args, I18N.get("JUMPWorkbench.jump"), new DeeJUMPConfiguration(), splashPanel,
					progressMonitor);
		} catch (Throwable t) {
	...
	public static void main(String[] args, String title, Setup setup,
		...
		try {
			...
			DeeJUMPWorkbench workbench = new DeeJUMPWorkbench(title, args, APP_ICON,
					splashWindow, taskMonitor);
			
			setup.setup(workbench.getContext());

			workbench.getContext().getWorkbench().getPlugInManager().load();
	...
	private static void initLookAndFeel() throws Exception {
        if ( LangUtil.ifNull( System.getProperty( "initLookAndFeel" ), "true" ).toString().equalsIgnoreCase(
                                                                                                             "false" ) ) {
            return;
        }
...
*/
	/*public PlugInManager _getPlugInManager() {
