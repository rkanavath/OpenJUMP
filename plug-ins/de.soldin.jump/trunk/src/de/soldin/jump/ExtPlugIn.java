...
// adds i18n support
public abstract class ExtPlugIn implements PlugIn {
	...
	private void initResourceBundle() {
		...
		ExtClassLoader ecl;
		// already using an extension cl
		if (this.getClass().getClassLoader() instanceof ExtClassLoader) {
			ecl = (ExtClassLoader) this.getClass().getClassLoader();
		}
		// use an ecl to find thing in lib/ext/<pluginid> folder
		else {
			ecl = new ExtClassLoader();
			ecl.add(ecl.getLibFolder(this.getClass(), pluginid));
		}
		rb = ResourceBundle.getBundle(i18nPath, Locale.getDefault(), ecl);
