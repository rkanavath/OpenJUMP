...
public class AnalysisExtent {
	...
	public AnalysisExtent(final ILayer layer) {
		...
		if (layer instanceof IRasterLayer) {
			...
			m_dCellSizeX = rasterLayer.getLayerGridExtent().getCellSizeX();
			m_dCellSizeY = rasterLayer.getLayerGridExtent().getCellSizeY();
			recalculateNXAndNY();
		}
	...
	/**
	 * Returns true if the given 3D point falls within the 3D area (volume) covered by this extent
	 * 
	 * @param x
	 *                the x coordinate of the point
	 * @param y
	...
	public boolean fitsIn(final AnalysisExtent extent) {
		...
		double dOffsetRows;
		double dOffsetZ;
		final double MIN_DIF_XY = 0.0000000000001; //TOLERANCE: must be very small for lat/lon.
		final double MIN_DIF_Z = 0.00001; //TOLERANCE: can be larger for depth (Z).
		...
		bFitsX = (dOffsetCols - Math.floor(dOffsetCols + 0.5) < MIN_DIF_XY);

		dOffset = Math.abs(extent.getYMax() - this.getYMax());
		dOffsetRows = dOffset / this.getCellSizeY();
		bFitsY = (Math.abs(dOffsetRows - Math.floor(dOffsetRows + 0.5)) < MIN_DIF_XY);

		dOffset = Math.abs(extent.getZMax() - this.getZMax());
		dOffsetZ = dOffset / this.getCellSizeZ();
		bFitsZ = (Math.abs(dOffsetZ - Math.floor(dOffsetZ + 0.5)) < MIN_DIF_Z);
	...
	public GridCell getGridCoordsFromWorldCoords(final Point2D pt) {
		...
		final int x = (int) Math.floor((pt.getX() - m_dXMin) / m_dCellSizeX);
		final int y = (int) Math.floor((m_dYMax - pt.getY()) / m_dCellSizeY);
	...
	public String toString() {
		final String s = Double.toString(m_dXMin) + ", " + Double.toString(m_dYMin) + ", " + Double.toString(m_dZMin) + ", "
				...
				+ Double.toString(m_dCellSizeX) + ", " + Double.toString(m_dCellSizeY) 
