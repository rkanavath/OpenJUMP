...
/**
 * A parameter representing an image layer.
 * Basically, this is a multi-band raster layer.
 * In the GRASS GIS interface, it drops in as a replacement
 * for GRASS imagery groups.
 * 
 * It is possible to treat this type of parameter like a raster parameter:
 * Just use {@link getParameterValueAsRasterLayer}.
 * 
 * @author benducke
 * 
 */
public class ParameterImageLayer
         extends
         ParameterDataObject {
   ...
   @Override
   public IRasterLayer getParameterValueAsImageLayer() throws NullParameterValueException {

      if (m_ParameterValue != null) {
         return (IRasterLayer) m_ParameterValue;
      }
      if (((AdditionalInfoImageLayer) m_ParameterAdditionalInfo).getIsMandatory()) {
   ...
   public IRasterLayer getParameterValueAsRasterLayer() throws NullParameterValueException {
	   return (getParameterValueAsImageLayer());
   }
   ...
   public boolean setParameterAdditionalInfo(final AdditionalInfo additionalInfo) {
      ...
      if (additionalInfo instanceof AdditionalInfoImageLayer) {
   ...
   public static Parameter deserialize(final KXmlParser parser) throws XmlPullParserException, IOException {
      ...
      final ParameterImageLayer param = new ParameterImageLayer();
      final AdditionalInfoImageLayer ai = new AdditionalInfoImageLayer(bMandatory);
