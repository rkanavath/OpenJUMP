...
public class ASCII3DFileTools {
	...
	public static I3DRasterLayer readFile(final File file) throws IOException, NumberFormatException {
		...
		String s = new String();
		int iX, iY, iZ;
		double dCellsizeX, dCellsizeY, dCellsizeZ;
		double dX, dY, dZ;
		...
		s = fin.readLine();
		sTokens = s.split(" ");
		dCellsizeX = Double.parseDouble(sTokens[0]);
		dCellsizeY = Double.parseDouble(sTokens[1]);
		dCellsizeZ = Double.parseDouble(sTokens[2]);

		final AnalysisExtent ae = new AnalysisExtent();
		ae.setCellSizeX(dCellsizeX);
		ae.setCellSizeZ(dCellsizeZ);
		ae.setXRange(dX, dX + iX * dCellsizeX, true);
		ae.setYRange(dY, dY + iY * dCellsizeY, true);
	...
	public static boolean writeFile(final I3DRasterLayer layer,
		...
		try {
			...
			out.write(Double.toString(layer.getCellSizeX()) + " " 
					+ Double.toString(layer.getCellSizeX()) + " " 
					+ Double.toString(layer.getCellSizeZ()) + "\n");
