...
public abstract class GeoAlgorithm {
    ...
    /**
     * A (list of) PID(s) for this algorithms. The list will only be non-null if
     ...
     * of methods ({@link #addOutputRasterLayer}, {@link #addOutputVectorLayer},
     * etc.)
     * 
     */
     ...
     *            The output factory to use to generate new data objects
     * @param outputMap
     *            Maps {@link IDataObject} algorithm output names to the desired
     ...
     * no layer is used. (see {@link OuputFactory#getDefualtCRS}). Sextante does
     ...
     * output layers if a layer is passed and the current output CRS is not set.
     */
            ...
            for (i = 0; i < m_Parameters.getNumberOfParameters(); i++) {
                try {
                    if (m_Parameters.getParameter(i).getParameterTypeName()
                        ...
                        final AdditionalInfoMultipleInput additionalInfo = (AdditionalInfoMultipleInput) m_Parameters
    ...
    /**
     * Sets the current step in the progress bar, based on the ratio between the
     ...
     */
    protected boolean setProgress(final int iStep, final int iTotalNumberOfSteps) {
    ...
    /**
     * 
     * @return true if the algorithm requires parameters other than input layers
     *         or tables
     */
    ...
    /**
     * @return The number of image layers needed to run the algorithm
     */
    public int getNumberOfImageLayers() {

        return m_Parameters.getNumberOfImageLayers();
    ...
    /**
     * 
     * @return true if the user can define the analysis region to be used by the
     *         algorithm
     */
    ...
    /**
     * Use this method to indicate that the user can define the analysis region
     * extent. When creating the corresponding panel for the algorithm, it will
     * contain a "Output region" tab, so the user can define the extent he
     * prefers
     * 
     * @param bUserCanDefineAnalysisExtent
     *            true if the algorithm generates new raster layers
     */
    public void setUserCanDefineAnalysisExtent(
            final boolean bUserCanDefineAnalysisExtent) {
    ...
    /**
     * Use this method to indicate that the algorithm is determinated (i.e. the
     * number of steps to complete it is known)
     * 
     * @param bDeterminated
    ...
    /**
     * Use this method to indicate that the algorithm is to be executed as a
     * silent background tasqk.
     * 
     * @param bSilent
     *            true if it is a "silent" algorithm
     */
    public void setIsSilentProcess(final boolean bSilent) {

        m_bIsSilentProcess = bSilent;

    }

    /**
     * 
     * @return true if the algorithm is determinated (i.e. the number of steps
     *         to complete it is known)
     */
    ...
    /**
     * 
     * @return true if the algorithm is a background task without a GUI monitor
     */
    public boolean isSilentProcess() {
        return m_bIsSilentProcess;
    }
    ...
    /**
     * Use this method to add a new output object. Use it if, for instance, you
     * have a layer created not within the geoalgorithm using a getNewXXXX
     * method, but in a different place, like, for example, calling another
     * method.
     * 
     * @param output
    ...
    protected void addOutputObject(final String sName,
            final String sDescription, final IOutputChannel channel,
            final Object obj) {
        Output out;
    ...
    /**
     * Returns the output channel associated with the given output. If it is
     * null, it returns a temporary output channel, which is a
     * {@link es.unex.sextante.dataObjects.FileOutputChannel} with a temporary
     * filename. Returns null if no output with that name exists.
     * 
     * @param sName
     *            the name of the output from which to retrieve the output
     *            channel
    ...
    protected IOutputChannel getOutputChannel(final String sName) {
        ...
        try {
            ...
            if (channel == null) {
                channel = new FileOutputChannel(
                        m_OutputFactory.getTempFilename(out));
                out.setOutputChannel(channel);
                return channel;
            } else if (channel instanceof FileOutputChannel) {
                final String sFilename = getOutputFilename(out);
                if (sFilename == null) {
                    ((FileOutputChannel) channel).setFilename(m_OutputFactory
                            .getTempFilename(out));
                    return channel;
                } else {
    ...
    /**
     * Returns the filename associated with a given output, which must have a
     * file-based output channel. If the output filename matches any of the
     * valid extensions for the type of output (from the ones obtained from the
     * current output factory), it returns that filename. Otherwise, it adds the
     * default extension for the type of output. If the filename is null, it
     * returns null.
     * 
     * @param out
     *            an output object
     * @return a checked filename for the passed output object. returns null if
     *         the output object filename is null or the output channel
     *         associated with the output is not file-based
     */
    ...
    /**
     * This method creates a new vector layer and adds it to the set of output
     * objects of the algorithm. Use this when your algorithm generates a new
     * vector layer and you have to create it.
     * 
     * @param sName
     *            The name of the layer. Has to be the same that you used to
     *            define this output in the
     * @see {@link #defineCharacteristics()} method.
     * @param sDescription
     *            the long description of the output. This is the one usually
     *            used to describe the layer when added to a GIS GUI
     * @param iShapeType
     *            The shape type. See
    ...
    protected IVectorLayer getNewVectorLayer(final String sName,
            final String sDescription, final int iShapeType,
            final Class[] types, final String[] sFields)
            throws GeoAlgorithmExecutionException {

        try {
            final OutputVectorLayer out = (OutputVectorLayer) m_OutputObjects
                    .getOutput(sName);
            IVectorLayer layer;
            final IOutputChannel channel = getOutputChannel(sName);
            layer = m_OutputFactory.getNewVectorLayer(
                    getLayerName(sName, sDescription), iShapeType, types,
                    sFields, channel, m_CRS);
            addOutputVectorLayer(sName, sDescription, iShapeType, channel,
                    layer, out.getInputLayerToOverwrite());
            return layer;
        } catch (final Exception e) {
            throw new GeoAlgorithmExecutionException(e.getMessage());
        }
    ...
    protected IVectorLayer getNewVectorLayer(final String sName,
            final String sDescription, final int iShapeType,
            final Class[] types, final String[] sFields, final int[] fieldSize)
            throws GeoAlgorithmExecutionException {
        ...
        try {
            ...
            layer = m_OutputFactory.getNewVectorLayer(
                    getLayerName(sName, sDescription), iShapeType, types,
                    sFields, channel, m_CRS, fieldSize);
            addOutputVectorLayer(sName, sDescription, iShapeType, channel,
                    layer, out.getInputLayerToOverwrite());
            return layer;
        } catch (final Exception e) {
    ...
    private String getLayerName(final String sName, final String sDescription) {
    ...
    /**
     * This method creates a new vector layer but does not add it to the set of
     * output objects of the algorithm. If you are going to read data from this
     * layer, it is recommended to close the layer, call the postProcess()
     * method and then open it again, since some implementations might not
     * support reading and writing features at the same time.
     * 
     * @param iShapeType
     *            The shape type. See
     * @see {@link IVectorLayer} for more info about valid values
    ...
    protected IVectorLayer getTempVectorLayer(final int iShapeType,
        ...
        return m_OutputFactory.getNewVectorLayer("", iShapeType, types,
                sFields, channel, m_CRS);

    }

    /**
     * Returns the path of the folder in the file system in which temporary
     * layers can be created. Normally the individual GIS bindings should take
     * care of supplying a valid path. If this does not happen, then we fall
     * back to using "java.io.tmpdir", which is the JVM's choice of temp folder
     * location (normally the OS default).
     * 
     * @return (String) the full path to a folder for temporary file storage
     */
    public static String getTempFolder() {
        ...
        File clean = new File(dir);
        dir = clean.getAbsolutePath();
        clean = null;
        return (dir);
    }
    ...
    protected IRasterLayer getNewRasterLayer(final String sName,
            final String sDescription, final int iDataType, final int iBands)
            throws UnsupportedOutputChannelException {

        final IOutputChannel channel = getOutputChannel(sName);

        final IRasterLayer newLayer = m_OutputFactory.getNewRasterLayer(
                getLayerName(sName, sDescription), iDataType, m_AnalysisExtent,
    ...
    /**
     * This method creates a new monoband raster layer and adds it to the set of
     * output objects of the algorithm. Use this when your algorithm generates a
     * new raster layer and you have to create it. The grid extent is taken from
     * the algorithm analysis extent (@see {@link #getAnalysisExtent()}
     * 
     * @param sName
     *            The name of the layer. Has to be the same that you used to
     *            define this output in the
     ...
     */
    protected IRasterLayer getNewRasterLayer(final String sName,
            final String sDescription, final int iDataType)
            throws UnsupportedOutputChannelException {
     ...
     * This method creates a new raster layer and adds it to the set of output
     * objects of the algorithm. Use this when your algorithm generates a new
     * raster layer and you have to create it.
     * 
     * @param sName
     *            The name of the layer. Has to be the same that you used to
     *            define this output in the
    ...
    protected IRasterLayer getNewRasterLayer(final String sName,
            final String sDescription, final int iDataType,
            final AnalysisExtent extent, final int iBands)
            throws UnsupportedOutputChannelException {

        final IOutputChannel channel = getOutputChannel(sName);

        final IRasterLayer newLayer = m_OutputFactory.getNewRasterLayer(
                getLayerName(sName, sDescription), iDataType, extent, iBands,
                channel, m_CRS);
     ...
     * This method creates a new monoband raster layer and adds it to the set of
     * output objects of the algorithm. Use this when your algorithm generates a
     * new raster layer and you have to create it.
     * 
     * @param sName
     *            The name of the layer. Has to be the same that you used to
     *            define this output in the
    ...
    protected IRasterLayer getNewRasterLayer(final String sName,
        ...
        final IRasterLayer newLayer = m_OutputFactory.getNewRasterLayer(
                getLayerName(sName, sDescription), iDataType, extent, 1,
                channel, m_CRS);
    ...
    protected IRasterLayer getTempRasterLayer(final int iDataType,
        ...
        final IRasterLayer newLayer = m_OutputFactory.getNewRasterLayer("",
                iDataType, extent, iBands, channel, m_CRS);
    ...
    protected IRasterLayer getTempRasterLayer(final int iDataType,
        ...
        final IRasterLayer newLayer = m_OutputFactory.getNewRasterLayer("",
                iDataType, extent, 1, channel, m_CRS);
    ...
    protected I3DRasterLayer getNew3DRasterLayer(final String sName,
        ...
        final I3DRasterLayer newLayer = m_OutputFactory.getNew3DRasterLayer(
                getLayerName(sName, sDescription), iDataType, extent, channel,
                m_CRS);
    ...
    protected ITable getNewTable(final String sName, final String sDescription,
            final Class[] types, final String[] sFields)
            throws UnsupportedOutputChannelException {

        final IOutputChannel channel = getOutputChannel(sName);

        final ITable table = m_OutputFactory.getNewTable(
                getLayerName(sName, sDescription), types, sFields, channel);
    ...
    protected void addOutputRasterLayer(final String sName,
            final String sDescription, final int iBands,
            final IOutputChannel channel, final IRasterLayer layer) {
    ...
    /**
     * Adds a new raster layer to the output set. The value of the output is
     * null, and so is the filename. Use this method in the
     * DefineCharacteristics() method of the algorithm, to define the outputs
     * expected.
     * 
     * @param sName
     *            The name to identify the layer in the set
    ...
    /**
     * Adds a new monoband raster layer to the output set. The value of the
     * output is null, and so is the filename. Use this method in the
     * DefineCharacteristics() method of the algorithm, to define the outputs
     * expected.
     * 
     * @param sName
    ...
    /**
     * Adds a new 3D raster layer to the output set. The value of the output is
     * null, and so is the filename. Use this method in the
     * DefineCharacteristics() method of the algorithm, to define the outputs
     * expected.
     * 
     ...
     *            the name of the input param (another vector layer) that this
     *            output can overwrite
     * 
     */
    protected void addOutputVectorLayer(final String sName,
            final String sDescription, final int iType,
            final IOutputChannel channel, final IVectorLayer layer,
    ...
    /**
     * Adds a new vector layer to the output set. The value of the output is
     * null, and so is the filename. Use this method in the
     * DefineCharacteristics() method of the algorithm, to define the outputs
     * expected.
     * 
    ...
    protected void addOutputVectorLayer(final String sName,
        ...
        addOutputVectorLayer(sName, sDescription, iType, null, null,
                sInputLayerToOverwrite);
     ...
     * null, and so is the filename. Use this method in the
     * DefineCharacteristics() method of the algorithm, to define the outputs
     * expected. The shape type is set to undefined
     * 
     * @param sName
     *            The name to identify the layer in the set
     * @param sDescription
    ...
    protected void addOutputVectorLayer(final String sName,
        ...
        addOutputVectorLayer(sName, sDescription,
                OutputVectorLayer.SHAPE_TYPE_UNDEFINED, null, null, null);
    ...
    /**
     * Adds a new table to the output set. The value of the output is null, and
     * so is the filename. Use this method in the DefineCharacteristics() method
     ...
     *            The description of the layer
     */
    protected void addOutputTable(final String sName, final String sDescription) {
    ...
    protected void addOutputChart(final String sName,
            final String sDescription, final ChartPanel chart) {
    ...
    /**
     * Adds a new chart to the output set. The value of the output is null. Use
     * this method in the DefineCharacteristics() method of the algorithm, to
     * define the outputs expected.
     * 
     * @param sName
     ...
     */
    protected void addOutputChart(final String sName, final String sDescription) {
    ...
    /**
     * Adds a new image to the output set. The value of the output is null. Use
     * this method in the DefineCharacteristics() method of the algorithm, to
     * define the outputs expected.
     * 
     * @param sName
     ...
     *            The description of the output
     */
    protected void addOutputImage(final String sName, final String sDescription) {
     ...
     */
    protected void addOutputText(final String sName, final String sDescription,
            final String sText) {
    ...
    /**
     * Adds a new text numerical value to the output set. The value of the
     * output is null. Use this method in the DefineCharacteristics() method of
     * the algorithm, to define the outputs expected.
     * 
     * @param sName
     *            The name to identify the output in the set
     * @param sDescription
    ...
    /**
     * Adds a new text string to the output set. The value of the output is
     * null. Use this method in the DefineCharacteristics() method of the
     * algorithm, to define the outputs expected.
     * 
     * @param sName
     *            The name to identify the output in the set
     * @param sDescription
    ...
    public String getCommandLineName() {
        ...
        final String sCommandName = sClass.substring(iLast + 1, sClass.length()
                - "Algorithm".length());
     ...
     */
    public GeoAlgorithm getNewInstance() throws InstantiationException,
    ...
    /**
     * 
     * @return true if the algorithm is suitable for a modeling process. This
     *         should be based on how well-defined the outputs are. If, for
     *         instance, the number of output layers cannot be known in advance,
    ...
    /**
     * Any preprocessing needed before executing the algorithm as part of a
     * model should be carried on in this method. Also, this method should be
     * used to do any extra checking, to ensure that parameter values are
     * correct. If not, an exception should be thrown. If data are not yet
     * available (another algorithm in the model has to be run before this one),
     * this method should return false. Otherwise, should return true if the
     * algorithm can be run after being preprocessed.
     * 
     * @param model
     *            the model from which the algorithm is being executed
     * @throws GeoAlgorithmExecutionException
     *             if the parameters are not correct and the algorithm cannot be
     *             executed
     * @returns true if the algorithm can be executed after being preprocessed
     */
    public boolean preprocessForModeller(final Object model)
            throws GeoAlgorithmExecutionException {
    ...
    /**
     * Returns the number of new objects (layers and tables) generated by the
     * algorithm
     * 
     * @return the number of new objects (layers and tables) generated by the
     *         algorithm
     */
    ...
    protected String getCommandLineParametersHelp(final String sFirstLine) {
        ...
        for (i = 0; i < params.getNumberOfParameters(); i++) {
            ...
            if (param instanceof ParameterDataObject) {
                ...
                try {
                    ai = (AdditionalInfoDataObject) param
            ...
            if (param instanceof ParameterTableField) {
                ...
                try {
                    ai = (AdditionalInfoTableField) param
                            .getParameterAdditionalInfo();
            ...
            if (param instanceof ParameterFixedTable) {
                ...
                try {
                    ai = (AdditionalInfoFixedTable) param
                            .getParameterAdditionalInfo();
                ...
                try {
                    ai = (AdditionalInfoMultipleInput) param
                            .getParameterAdditionalInfo();
    ...
    /**
     * Returns the command line expression that would create this instance of
     * the algorithm
     * 
    ...
    public String[] getAlgorithmAsCommandLineSentences() {
        ...
        for (i = 0; i < m_Parameters.getNumberOfParameters(); i++) {
            final Parameter param = m_Parameters.getParameter(i);
            if (param != null) {
                String sCLP = new String();
                sCLP = (param.getCommandLineParameter());
                if (sCLP != null)
                    sb.append(sCLP + ", ");
                else
                    m_bIncompleteParams = true;
            }
        }
        ...
        sAlg = sAlg.replaceAll("(\\r|\\n)", "");

        if (getUserCanDefineAnalysisExtent() && getAnalysisExtent() != null) {
        ...
        /* Raise a warning if one of the parameters was NULL. */
        if (m_bIncompleteParams == true) {
            Sextante.addWarningToLog(Sextante.getText("Warn_empty_input_param"));
        }
     ...
     * 
     * @return true if there are any params with a NULL value
     */
    public boolean hasIncompleteParams() {

        return m_bIncompleteParams;
    ...
    /**
     * Returns true if the algorithm requires vector layers as individual input
     * (i.e. not as a multiple input)
     * 
    ...
    /**
     * Returns true if the algorithm requires image layers to run
     * 
     * @see ParametersSet#requiresRasterLayers()
     * @return true if the algorithm requires image to run
     */
    public boolean requiresImageLayers() {

        return m_Parameters.requiresImageLayers();
    ...
    /**
     * Returns true if the algorithm requires raster layers as individual input
     * (i.e. not as a multiple input)
     * 
    ...
    /**
     * Returns true if the algorithm requires raster bands as multiple input
     * (i.e. the parameter set contains a multiple input parameter of type band)
     * 
    ...
    /**
     * Returns false if there are not enough raster data in the given array to
     * run the algorithm
     * 
     * @param objs
     *            an array of available data objects
     * @return false if the algorithm requires more raster layers than the ones
     *         in the data objects array. True if they suffice
     */
    ...
    /**
     * Returns false if there are not enough image data in the given array to
     * run the algorithm
     * 
     * @param objs
     *            an array of available data objects
     * @return false if the algorithm requires more image layers than the ones
     *         in the data objects array. True if they suffice
     */
    public boolean meetsImageRequirements(final Object[] objs) {

        if (objs == null) {
            return false;
        }

        if (requiresImageLayers()) {
            for (final Object element : objs) {
                if (element instanceof IRasterLayer) {
                    if (((IRasterLayer) element).getBandsCount() > 1) {
                        return true;
                    }
                }
            }
    ...
    /**
     * Returns false if there are not enough 3D raster data in the given array
    ...
    /**
     * Returns false if there are not enough vector data in the given array to
     * run the algorithm
     * 
     * @param objs
     *            an array of available data objects
     * @return false if the algorithm requires more (or different, i.e other
     *         shape type) vector layers than the ones in the data objects
     *         array. True if they suffice
     */
    public boolean meetsVectorRequirements(final Object[] objs) {
        ...
        if (requiresVectorLayers()) {
            ...
            if ((requiresPointVectorLayers() && !bPointLayers)
                    || (requiresLineVectorLayers() && !bLineLayers)
    ...
    /**
     * Returns false if there are not enough table objects in the given array to
     * run the algorithm
     * 
     * @param objs
     *            an array of available data objects
     * @return false if the algorithm requires more tables than the ones in the
     *         data objects array. True if they suffice
     */
    ...
    /**
     * Returns true if the algorithm could be executed with the given data
     * objects.
     * 
     * @param objs
     *            an array of available data objects
     * @return true if the algorithm could be executed with the given data
     *         objects.
     */
    public boolean meetsDataRequirements(final Object[] objs) {
        ...
        return meets3DRasterRequirements(objs) && meetsRasterRequirements(objs)
                && meetsVectorRequirements(objs)
                && meetsTableRequirements(objs) && meetsImageRequirements(objs);
    ...
    /**
     * Returns true if the extent has been set automatically by the algorithm,
     * false if it was introduced by the user
     * 
     * @return true if the extent has been set automatically by the algorithm,
     *         false if it was introduced by the user
     */
    ...
    /**
     * Return true if the algorithm can define the output extent from its
     * required input layers. False otherwise.
     * 
     * @return true if the algorithm can define the output extent from its
     *         required input layers. False otherwise.
     */
    public boolean canDefineOutputExtentFromInput() {
        ...
        if (m_OutputObjects.getRasterLayersCount() != 0) {
            return requiresRasterLayers() || requiresImageLayers()
                    || requires3DRasterLayers();
        }
    ...
    /**
     * Until we find a way of automating this, 3d algorithms should overwrite
     * this method to return true. Otherwise, it will return false by default.
     * This method is used to know whether to show a 3D Analysis Extent
     * definition panel in the algorithm dialog or a regular 2D one
     * 
    ...
    /**
     * Returns true if the algorithm should be shown in a SEXTANTE component,
     * false otherwise. Use this to programatically hide algorithms if you do
     * not want them to be shown and used for some reason.
     * 
     * @return true if the algorithm should be shown in a SEXTANTE component,
     *         false otherwise.
     */
    ...
    /**
     * 
     * Adds the PID (int) of a process to this algorithms's list of PIDs. This
     * 
     * @param PID
     *            PID (int) to add to list of external process PIDs
     */
    public void addPID(int PID) {
        if (m_lPID == null) {
            m_lPID = new ArrayList<Integer>();
        }
        m_lPID.add(new Integer(PID));
        // Notify TaskMonitor of new PID:
        m_Task.setPIDList(m_lPID);
    }
}
