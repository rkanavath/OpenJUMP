...
public class Rectangle {
    ...
    */
   private float distanceSquared(final int dimension,
                                 final float point) {
      float distanceSquared = 0;
    ...
    *
    * @param r
    *                Rectangle to union with this rectangle, in order to compute the difference in area of the union and the
    *                original rectangle
    */
   public float enlargement(final Rectangle r) {
      final float enlargedArea = (Math.max(max[0], r.max[0]) - Math.min(min[0], r.min[0]))
