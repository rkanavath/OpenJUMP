...
public class GridBasicStatsAlgorithm extends GeoAlgorithm {
    ...
    public void defineCharacteristics() {
        ...
        try {
            m_Parameters.addInputRasterLayer(INPUT, Sextante.getText("Layer"),
                    true);
            addOutputText(STATS, Sextante.getText("Statistics"));
            addOutputNumericalValue(MEAN, Sextante.getText("Mean_value"));
            addOutputNumericalValue(MEAN_SQUARED,
                    Sextante.getText("Mean_squared_value"));
            addOutputNumericalValue(MIN, Sextante.getText("Minimum_value"));
            addOutputNumericalValue(MAX, Sextante.getText("Maximum_value"));
            addOutputNumericalValue(NODATA, Sextante.getText("NoDataValue"));
            addOutputNumericalValue(VARIANCE, Sextante.getText("Variance"));
            addOutputNumericalValue(SUM, Sextante.getText("Total_sum"));
            addOutputNumericalValue(COEF_OF_VARIATION,
                    Sextante.getText("Coefficient_of_variation"));
            addOutputNumericalValue(VALID_CELLS,
                    Sextante.getText("Valid_data_cells"));
            addOutputNumericalValue(NO_DATA_CELLS,
                    Sextante.getText("No_data_cells"));
    ...
    public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
        ...
        doc.addListElement(Sextante.getText("Mean_value") + ": "
                + df.format(stats.getMean()));
        doc.addListElement(Sextante.getText("Mean_squared_value") + ": "
                + df.format(stats.getRMS()));
        doc.addListElement(Sextante.getText("Minimum_value") + ": "
                + df.format(stats.getMin()));
        doc.addListElement(Sextante.getText("Maximum_value") + ": "
                + df.format(stats.getMax()));
        doc.addListElement(Sextante.getText("NoDataValue") + ": "
                + df.format(nodata));
        doc.addListElement(Sextante.getText("Variance") + ": "
                + df.format(stats.getVariance()));
        doc.addListElement(Sextante.getText("Total_sum") + ": "
                + df.format(stats.getSum()));
        doc.addListElement(Sextante.getText("Coefficient_of_variation") + ": "
                + df.format(stats.getCoeffOfVar()));
        doc.addListElement(Sextante.getText("Valid_data_cells") + ": "
                + Integer.toString(stats.getCount()));
        doc.addListElement(Sextante.getText("No_data_cells") + ": "
                + Integer.toString((iNX * iNY) - stats.getCount()));
        doc.closeUnorderedList();
        doc.close();

        addOutputText(STATS,
                Sextante.getText("Statistics") + "[" + input.getName() + "]",
                doc.getHTMLCode());
        ...
        addOutputNumericalValue(NODATA, nodata);
