...
public class RandomGeometryUtilities {
   ...
   public Coordinate nextCoordinate(final Envelope envelope) {	   
	 Double RangeX = Math.abs(envelope.getMaxX()-envelope.getMinX());
	 Double RangeY = Math.abs(envelope.getMaxY()-envelope.getMinY());
	 
	 Double X = envelope.getMinX() + ( RangeX * RAND.nextDouble() );
	 Double Y = envelope.getMinY() + ( RangeY * RAND.nextDouble() );
	 Double Z = 100.00 * RAND.nextDouble();
