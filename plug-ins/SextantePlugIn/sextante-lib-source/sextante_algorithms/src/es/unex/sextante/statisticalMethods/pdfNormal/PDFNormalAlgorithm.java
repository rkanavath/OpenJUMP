package es.unex.sextante.statisticalMethods.pdfNormal;
...
public class PDFNormalAlgorithm
         extends
            GeoAlgorithm {
   ...
   public void defineCharacteristics() {
      ...
      this.setName(Sextante.getText("Normal_probability_distribution"));
   ...
   public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
      ...
      final IRasterLayer result = getNewRasterLayer(PROBABILITY, Sextante.getText("Probability__normal"),
      ...
      for (y = 0; y < iNY && setProgress(y, iNY); y++) {
         for (x = 0; x < iNX; x++) {
            ...
            if (!window.isNoDataValue(dValue)) {
               if (bCDF) {
                  result.setCellValue(x, y, PDF.normalCDF(dMean, dStdDev, dValue));
               }
               else {
                  result.setCellValue(x, y, PDF.normal(dMean, dStdDev, dValue));
               }
            }
            else {
