...
public class FresnelLOSAlgorithm
   ...
   public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
      ...
      final double x = ge.getXMin() + m_Point.getX() * ge.getCellSizeX();
      final double y = ge.getYMax() - m_Point.getY() * ge.getCellSizeY();
      final double x2 = ge.getXMin() + m_Point2.getX() * ge.getCellSizeX();
      final double y2 = ge.getYMax() - m_Point2.getY() * ge.getCellSizeY();
   ...
   private void calculateLOS(int x,
      ...
      if (d > 0) {
         ...
         final double meanCellSize = (m_DEM.getWindowCellSizeX()+m_DEM.getWindowCellSizeY())/2.0;

         dHeight += m_dHeight;
         dHeight2 += m_dHeight2;
         dAngle = Math.atan2(dHeight2 - dHeight, dist * meanCellSize);

         straightlineSerie.add(0, dHeight);
         straightlineSerie.add(dist * meanCellSize, dHeight2);
         ...
         while (id < dist) {
            ...
            if (!m_DEM.isNoDataValue(z)) {
               serie.add(id * meanCellSize, z);
               final double dDist = id * meanCellSize / Math.cos(dAngle);
               final double dDist2 = (dist - id) * meanCellSize / Math.cos(dAngle);
         ...
         fresnelSerie.add(dist * meanCellSize, dHeight2);

         final JFreeChart chart = ChartFactory.createXYLineChart(null, null, null, m_Dataset, 
