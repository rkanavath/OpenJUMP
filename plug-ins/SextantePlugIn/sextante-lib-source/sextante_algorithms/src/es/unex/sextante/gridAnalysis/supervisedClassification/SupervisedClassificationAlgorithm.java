...
public class SupervisedClassificationAlgorithm
   ...
   private AnalysisExtent getAdjustedGridExtent(final int iLayer) {
      ...
      dCellSizeX = m_Window[iLayer].getLayerGridExtent().getCellSizeX();
      dCellSizeY = m_Window[iLayer].getLayerGridExtent().getCellSizeY();
   ...
   private void doPolygonPart(final Geometry geom,
      ...
      for (i = 0; i < m_Window.length; i++) {
         ...
         xStart = (int) ((extent.getMinX() - ge.getXMin()) / ge.getCellSizeX()) - 1;
         if (xStart < 0) {
            xStart = 0;
         }

         xStop = (int) ((extent.getMaxX() - ge.getXMin()) / ge.getCellSizeX()) + 1;
         if (xStop >= iNX) {
            xStop = iNX - 1;
         }

         for (y = 0, yPos = ge.getYMax(); y < iNY; y++, yPos += ge.getCellSizeY()) {
            if ((yPos >= extent.getMinY()) && (yPos <= extent.getMaxY())) {
               ...
               for (iPoint = 0; iPoint < points.length; iPoint++) {
                  ...
                  if ((((pa.y <= yPos) && (yPos < pb.y)) || ((pa.y > yPos) && (yPos >= pb.y)))) {
                     ...
                     ix = (int) ((p.x - ge.getXMin()) / ge.getCellSizeX() + 1.0);
