...
GeoAlgorithm {
	...
	private void Do_Tarboton(final int x,
		...
		if (m_DEM.isNoDataValue(z)) {
			...
			for (i = 0, j = 1; i < 8; i++, j = (j + 1) % 8) {
				if ((i % 2) != 0) // i => diagonal
				{
					G = (z - zm[j]) / meanCellSize;
					H = (zm[j] - zm[i]) / meanCellSize;
				}
				else // i => orthogonal
				{
					G = (z - zm[i]) / meanCellSize;
					H = (zm[i] - zm[j]) / meanCellSize;
				}
				...
				if (H < 0.0) {
					...
					iSlope = (z - zm[((i % 2) != 0) ? i : j]) / (Math.sqrt(2.0) * meanCellSize);
				}
				else {
	...
	private void Do_LeastSquare(final int x,
		...
		if (Get_SubMatrix3x3(x, y, zm)) {
			...
			a = ((zm[2] + 2 * zm[5] + zm[8]) - (zm[0] + 2 * zm[3] + zm[6])) / (8 * meanCellSize);
			b = ((zm[6] + 2 * zm[7] + zm[8]) - (zm[0] + 2 * zm[1] + zm[2])) / (8 * meanCellSize);
