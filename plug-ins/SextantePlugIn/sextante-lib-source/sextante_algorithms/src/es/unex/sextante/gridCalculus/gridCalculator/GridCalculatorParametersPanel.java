...
public class GridCalculatorParametersPanel
   ...
   private void populateTree() {
      ...
      for (i = 0; i < layers.length; i++) {
         sName = Sextante.getText("Cell_size_x_[") + layers[i].getName() + "]";
         dCellsize = layers[i].getLayerCellSizeX();
         m_Constants.put(sName, " " + Double.toString(dCellsize) + " ");
      }
      for (i = 0; i < layers.length; i++) {
          sName = Sextante.getText("Cell_size_y_[") + layers[i].getName() + "]";
          dCellsize = layers[i].getLayerCellSizeY();
          m_Constants.put(sName, " " + Double.toString(dCellsize) + " ");
       }
