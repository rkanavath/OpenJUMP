...
public class CostInRoutesAnisotropicAlgorithm
   ...
   private static final double NO_DATA    = SextanteGUI.getDefaultNoDataValue();
   ...
   public void defineCharacteristics() {
      ...
      try {
         m_Parameters.addInputVectorLayer(ROUTES, Sextante.getText("Routes"), 
         ...
         m_Parameters.addNumericalValue(KFACTOR, Sextante.getText("K"), 2, 
   ...
   private void processSegment(double x,
      ...
      if ((dx > 0.0) || (dy > 0.0)) {
         if (dx > dy) {
            dx /= m_Cost.getWindowCellSizeX();
            n = dx;
            dy /= dx;
            dx = m_Cost.getWindowCellSizeX();
         }
         else {
            dy /= m_Cost.getWindowCellSizeY();
            n = dy;
            dx /= dy;
            dy = m_Cost.getWindowCellSizeY();
         }
   ...
   private void addPoint(final double x,
      ...
      if (m_iPoints == 0) {
         ...
         iDX = iX - m_iLastX;
         iDY = iY - m_iLastY;
         m_dDist[m_iCurrentRoute] += Math.sqrt(iDX * iDX + iDY * iDY) * 
        		 ((m_Cost.getWindowCellSizeX()+m_Cost.getWindowCellSizeY())/2.0);
