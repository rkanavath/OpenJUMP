...
GeoAlgorithm {
	...
	public boolean processAlgorithm() throws GeoAlgorithmExecutionException {
		...
		try {
			...
			final double meanCellSize = (m_DEM.getLayerCellSizeX()+m_DEM.getLayerCellSizeY())/2.0;
			m_searchRadius = (int) (m_worldSearchRadius / meanCellSize);
	...
	private boolean calculateVisibility() {
		...
		try {
			...
			final double meanCellSize = (m_DEM.getLayerCellSizeX()+m_DEM.getLayerCellSizeY())/2.0;
			m_minAllowedAngle = new LineSegment(0.0, 0.0, m_worldSearchRadius, meanCellSize).angle();
