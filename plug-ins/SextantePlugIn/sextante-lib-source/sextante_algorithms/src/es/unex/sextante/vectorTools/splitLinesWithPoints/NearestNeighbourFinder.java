...
public class NearestNeighbourFinder {
   ...
   public NearestNeighbourFinder(final IVectorLayer layer,
      ...
      m_Attributes = new Object[m_iPoints];
      i = 0;
      ...
      while (iter.hasNext() && !task.isCanceled()) {
         ...
         m_dX[i] = x;
         m_dY[i] = y;
         m_Attributes[i] = feature.getRecord().getValues();
   ...
   public PtAndAttributes[] getClosestPoints(final LineSegment seg,
                                             final double dDistance) {

      int i;
      int iID;
      final Envelope search = new Envelope(seg.p0, seg.p1);
      search.expandBy(dDistance);
      final List<Integer> list = m_Tree.query(search);
      final int iSize = list.size();
      final PtAndAttributes pts[] = new PtAndAttributes[iSize];
      for (i = 0; i < iSize; i++) {
         iID = list.get(i).intValue();
         pts[i] = new PtAndAttributes(m_dX[iID], m_dY[iID], iID, (Object[]) m_Attributes[iID]);
      }
