...
public class GridCalculatorModelerParametersPanel
   ...
   public boolean assignParameters(final HashMap map) {
	  ...
	  // Possible input elements are elements of type "raster" and "image"
	  int numRasters = getElementsOfClass(ModelElementRasterLayer.class, false).length;
	  int numImages = getElementsOfClass(ModelElementImageLayer.class, false).length;
	  ObjectAndDescription[] layers = new ObjectAndDescription[numRasters+numImages];
	  for ( int i = 0; i < numRasters; i ++ ) {
		  layers [i] = getElementsOfClass(ModelElementRasterLayer.class, false)[i];
	  }
	  int elem = 0;
	  for ( int i = numRasters; i < (numRasters+numImages); i ++ ) {
		  layers [i] = getElementsOfClass(ModelElementImageLayer.class, false)[elem];
		  elem ++;
	  }
