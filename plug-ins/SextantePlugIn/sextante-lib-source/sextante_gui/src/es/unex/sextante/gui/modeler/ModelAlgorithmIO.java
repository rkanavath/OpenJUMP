...
public class ModelAlgorithmIO {
    ...
    public static boolean saveAsJava(final ModelAlgorithm alg, final File file) {
    ...
    public static boolean save(final ModelerPanel panel, final File file) {
        ...
        try {
            ...
            for (i = 0; i < algs.size(); i++) {
                ...
                serializer.attribute(null, ALG_CMD_LINE_NAME,
                        alg.getCommandLineName());
                serializer.attribute(null, DESCRIPTION, alg.getDescription());
                serializer.attribute(null, RED,
                        Integer.toString(alg.getColorR()));
                serializer.attribute(null, GREEN,
                        Integer.toString(alg.getColorG()));
                serializer.attribute(null, BLUE,
                        Integer.toString(alg.getColorB()));
                serializer.attribute(null, ALPHA,
                        Integer.toString(alg.getColorAlpha()));
                serializer.attribute(
                        null,
                        COORDS,
                        getCoordsAsString(panel.getModelGraphPanel()
                                .getCoords(), sKey));
            ...
            for (i = 0; i < iCount; i++) {
                ...
                final String sCoords = getCoordsAsString(panel
                        .getModelGraphPanel().getCoords(), sKey);
            ...
            while (iter.hasNext()) {
                ...
                if (sKey.indexOf("INPUT") != 0) {
                    final ObjectAndDescription oad = (ObjectAndDescription) dao
                            .get(sKey);
                    serializer.text("\n");
                    serializer.text("\t\t");
                    ...
                    serializer.attribute(null, DESCRIPTION,
                            oad.getDescription());
    ...
    private static String getValueAsString(final Object ob) {
        ...
        /*
         * if (ob instanceof IModelElement){ return ob.getClass().toString(); }
         */
        else {
            return ob.toString();
        }
    ...
    public static ModelAlgorithm open(final File file, final HashMap dao,
            final ArrayList inputKeys, final HashMap coords) {
        ...
        try {
                ...
                while ((tag != XmlPullParser.END_DOCUMENT) && !bOut) {
                    ...
                    case XmlPullParser.START_TAG:
                        if (parser.getName().compareTo(MODEL) == 0) {
                            final String sName = parser.getAttributeValue("",
                                    NAME);
                            model.setName(sName);
                            final String sGroup = parser.getAttributeValue("",
                                    GROUP);
                            model.setGroup(sGroup);
                        } else if (parser.getName().compareTo(ALGORITHM) == 0) {
                            alg = getAlgorithm(parser.getAttributeValue("",
                                    ALG_CMD_LINE_NAME));
                            sAlgName = parser.getAttributeValue("", KEY);
                            final String sCoords = parser.getAttributeValue("",
                                    COORDS);
                            if (alg == null) {
                                throw new ModelIOException();
                            }
                            sAlgDescription = parser.getAttributeValue("",
                                    DESCRIPTION);
                            ...
                            if (!sCoords.equals("null")) {
                                coords.put(sAlgName,
                                        getCoordsFromString(sCoords));
                            }
                        } else if (parser.getName().compareTo(DATA_OBJECT) == 0) {
                            final String sKey = parser.getAttributeValue("",
                                    KEY);
                            final String sValue = parser.getAttributeValue("",
                                    VALUE);
                            final String sDescription = parser
                            ...
                            final String sDescription = parser
                                    .getAttributeValue("", DESCRIPTION);
                            addOutputObject(sKey, sValue, sDescription, ooSet,
                                    dao);
                            ...
                            if (param == null) {
                                Sextante.getLogger().addError(
                                        "Failed to load model '"
                                                + file.getName() + "'.\n");
                                Sextante.getLogger().addError(
                                        "Unable to parse input parameter '"
                                                + parser.getAttributeValue("",
                                                        NAME) + "'.\n");
                                Sextante.getLogger()
                                        .addError(
                                                "Check that the model is not malformed and that all algorithm providers are available.\n");
                                return null;
                            }
                            if (sColorRed != null)
                            ...
                            if (sColorAlpha != null)
                                param.setColorAlpha(Integer
                                        .parseInt(sColorAlpha));
                            ps.addParameter(param);
                            inputKeys.add(param.getParameterName());
                            final ObjectAndDescription oad = new ObjectAndDescription(
                                    param.getParameterDescription(),
                                    ModelElementFactory
                                            .getParameterAsModelElement(param));
                            dao.put(param.getParameterName(), oad);
                            if (rect != null) {
                                coords.put(param.getParameterName(),
                                        getCoordsFromString(sCoords));
                            }
                        } else if (parser.getName().compareTo(ASSIGNMENT) == 0) {
                            final String sKey = parser.getAttributeValue("",
                                    KEY);
                            String sAssignment = parser.getAttributeValue("",
                                    ASSIGNEDTO);
                            if (sAssignment.equals("null")) {
                                sAssignment = null;
                            }
                            // System.out.println ("model.addInputAssignment("+
                            // sKey +"," + sAssignment +").\n");
            ...
            Sextante.getLogger().addError(
                    "Failed to load model '" + file.getName() + "'.\n");
            Sextante.getLogger()
                    .addError(
                            "Check that the model is not malformed and that all algorithm providers are available.\n");
            return null;
    ...
    private static void addOutputObject(final String sKey, final String sValue,
            final String sDescription, final OutputObjectsSet ooSet,
            final HashMap dao) {
        ...
        if (sValue.equals(OutputRasterLayer.class.toString())) {
            ...
            final ObjectAndDescription oad = (ObjectAndDescription) dao
                    .get(sKey);
            if (oad != null) {
                final ModelElementVectorLayer mevl = (ModelElementVectorLayer) oad
                        .getObject();
    ...
    public static ModelAlgorithm open(final File file, final ModelerPanel panel) {
    ...
    private static Rectangle2D getCoordsFromString(final String sCoords) {
        ...
        rect.setRect(Double.parseDouble(coords[0]),
    ...
    private static void addDataObject(final String sKey, final String sValue,
            final String sDescription, final HashMap dao) {

        if (sKey.indexOf("INNERPARAM") == 0) {
            if (sDescription.equals("Numerical Value")) {
                if (sValue.indexOf(".") != -1) {
                    dao.put(sKey, new ObjectAndDescription(sDescription,
                            new Double(Double.parseDouble(sValue))));
                } else {
                    dao.put(sKey, new ObjectAndDescription(sDescription,
                            new Integer(Integer.parseInt(sValue))));
                }
            }
            if (sDescription.equals("String") || sDescription.equals("Field")
                    || sDescription.equals("Filepath")) {
                dao.put(sKey, new ObjectAndDescription(sDescription, sValue));
                // System.out.println ("*** PUTTING: sKey=" + sKey +
                // ", SDescr.=" + sDescription + ", sValue='" + sValue +"'.\n");
            } else if (sDescription.equals("Selection")
                    || sDescription.equals("Band")) {
                dao.put(sKey, new ObjectAndDescription(sDescription,
                        new Integer(Integer.parseInt(sValue))));
                ...
                final FixedTableModel ftm = new FixedTableModel(sColNames,
                        iRows, false);
                ...
                dao.put(sKey, new ObjectAndDescription(sDescription,
                        new Boolean(sValue.equals("true"))));
            }
            ...
            } else if (sTokens[0].equals(ModelElementImageLayer.class
                    .toString())) {
                final ModelElementImageLayer meil = new ModelElementImageLayer();
                meil.setNumberOfBands(Integer.parseInt(sTokens[1]));
                dao.put(sKey, new ObjectAndDescription(sDescription, meil));
            } else if (sTokens[0].equals(ModelElementTable.class.toString())) {
                dao.put(sKey, new ObjectAndDescription(sDescription,
                        new ModelElementTable()));
            } else if (sTokens[0].equals(ModelElement3DRasterLayer.class
                    .toString())) {
                dao.put(sKey, new ObjectAndDescription(sDescription,
                        new ModelElement3DRasterLayer()));
            } else if (sTokens[0].equals(ModelElementNumericalValue.class
                    .toString())) {
                dao.put(sKey, new ObjectAndDescription(sDescription,
                        new ModelElementNumericalValue()));
            } else if (sTokens[0].equals(ModelElementVectorLayer.class
                    .toString())) {
    ...
    private static GeoAlgorithm getAlgorithm(
            final String sAlgorithmCommandLineName) {

        try {
            return Sextante.getAlgorithmFromCommandLineName(
                    sAlgorithmCommandLineName).getNewInstance();
        } catch (final Exception e) {
            return null;
        }
    ...
    public static GeoAlgorithm[] loadModelsAsAlgorithms(
            ...
            for (i = 0; i < contents.length; i++) {
                ...
                try {
                    if (file.getName().endsWith("model")) {
                        ...
                        final HashMap dao = new HashMap();
                        final HashMap coords = new HashMap();
                        final ModelAlgorithm alg = open(file, dao, inputKeys,
                                coords);
                        if (alg != null) {
                            alg.getInputs().clear();
                            final Set set = dao.keySet();
