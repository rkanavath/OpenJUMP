...
public class GrassBlackList {
   ...
   private static final String[] ALGS = { "i.class","i.group", "i.in.spotvgt", "i.landsat.acca", 
		    "i.landsat.toar", "i.rectify", "i.spectral", "i.target", 
   		    "v.build.polylines", "v.build", "v.category", "v.convert", "v.db.connect",
   ...
   public static boolean isInBlackList(final String sName) {
	   if ( m_List.contains(sName)) {
		   Sextante.addWarningToLog("GRASS interface: Module " + sName + " disabled: blacklisted.");
		   return ( true );
	   }
	   return ( false );
   }
