...
public class RAlgorithm
   ...
   private String[] getImportCommands() throws GeoAlgorithmExecutionException {
      ...
      commands.add("x=c(" + Double.toString(m_AnalysisExtent.getXMin()) + "," + 
    		  Double.toString(m_AnalysisExtent.getXMax()) + ")");
      commands.add("y=c(" + Double.toString(m_AnalysisExtent.getYMin()) + "," + 
    		  Double.toString(m_AnalysisExtent.getYMax()) + ")");
      commands.add("xy <- cbind(x,y)");
      commands.add("boundingBox <- SpatialPoints(xy)");
      commands.add("cellsize=" + Double.toString(m_AnalysisExtent.getCellSizeX()));//TODO: BD: Set X/Y cell sizes individually!
