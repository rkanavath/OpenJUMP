...
IAlgorithmProvider {
	...
	/**
	 * Initializes this algorithm provider.
	 * This also means running GRASS 6 in the background to create XML description
	 * files for its modules if the provider is active and no description files are
	 * present on startup. 
	 */
	public void initialize() {
		GrassUtils.createTempMapsetName();
		// Create GRASS algorithm descriptions if required
		boolean active = Boolean.parseBoolean(SextanteGUI.getSettingParameterValue(SextanteGrassSettings.GRASS_ACTIVATE));
		boolean create = false;
		if ( getGrassDescriptionFolder() != null && getGrassDescriptionFolder().length() > 0 ) {
			File check = new File (getGrassDescriptionFolder());
			if ( check.exists() == false ) {
				// Description files folder does not exist: need to create descriptions!
				create = true;
			} else {				
				File[] files = check.listFiles(new FilenameFilter() {
				    public boolean accept(File dir, String name) {
				        return name.toLowerCase().endsWith(".xml");
				    }
				});
				if ( files != null && files.length == 0 ) {
					// Description files folder is empty: need to create descriptions!
					create = true;
				}
			}
		}
		if ( create == true && active == true ) {
			try {
				Sextante.getLogger().addInfo("Generating GRASS 6 algorithm descriptions on init.");
				createAlgorithmsDescriptionFiles();
			} catch (WrongGrassFolderException e) {				
				Sextante.getLogger().addError("Failed to generate XML description files for GRASS 6 algorithms on init.");
			}
		}		
		createAlgorithmsMap();
	}
	...
	public static int createAlgorithmsDescriptionFiles() throws WrongGrassFolderException {
			...
			if (files != null) {
				if (Sextante.isUnix() || Sextante.isMacOSX()) {
					for (int i = 0; i < files.length; i++) {
						// We currently support most Imagery (i.*), Vector (v.*) and 2D raster (r.*) modules
						if ( (files[i].startsWith("i.") || files[i].startsWith("v.") || files[i].startsWith("r.")) 
					...
					for (int i = 0; i < files.length; i++) {
						if ((files[i].endsWith(".exe") || files[i].endsWith(".bat"))
								...
								&& (files[i].startsWith("i.") || files[i].startsWith("v.") || files[i].startsWith("r.")) 
								&& !files[i].startsWith("r3.")
		...
		Sextante.addInfoToLog("GRASS interface: Done setting up GRASS.");
	...
	public static String getGrassDescriptionFolder() {		
		...
		final File base = new File (Sextante.getUserFolder() + File.separator + Sextante.DESC_FOLDER_BASE);
		if (!base.exists()) {
			base.mkdir();
		}      

		final String sPath = base + File.separator + Sextante.DESC_FOLDER_GRASS;
	...
	/**
	 * Logs a GRASS info message.
	 * Info messages are buffered and only written to the log file when
	...
	/**
	 * Commits all info messages of the current module's run to the log.
	 * 
	 * @param sLabel
	 * 			the label for the associated tree node in the message log window
	 */
	public static void publishMessage(final String sLabel) {
		Sextante.getLogger().addToLog(m_Message.toString(), "GRASS ", sLabel);
	}


	/**
	 * Add an informational message to the GRASS message log and display
	 * it in the progress monitor.
	 * 
	 * @param s
	 * 			message string
	 * @param alg
	 * 			GrassAlgorithm that emitted the message
	 */
	public static void addInfo(final String s, GrassAlgorithm alg) {
	...
	/**
	 * Immediately logs a GRASS error message.
	 * 
	 * @param s
	 */
	public static void addError(final String s) {
		if ( s != null ) {
			Sextante.addErrorToLog("GRASS interface: " + s);
		}
	}


	/**
	 * Logs a GRASS error message and shows it in the progress monitor.
	 * 
	 * @param s
	 * 			message string
	 * @param alg
	 * 			GrassAlgorithm that emitted the message
	 */
	public static void addError(final String s, GrassAlgorithm alg) {
		addError ( s );		
		if ( s != null && alg != null ) {
			alg.showError(s+"\n");
		}
	...
	/**
	 * Immediately logs a GRASS warning message.
	 * 
	 * @param s
	 */
	public static void addWarning(final String s) {
		if ( s != null ) {
			Sextante.addWarningToLog("GRASS interface: " + s);
		}
	}
	...
	/**
	 * Logs a GRASS warning message and shows it in the progress monitor.
	 * 
	 * @param s
	 * 			message string
	 * @param alg
	 * 			GrassAlgorithm that emitted the message
	 */
	public static void addWarning(final String s, GrassAlgorithm alg) {
		addWarning (s);
		if ( s != null && alg != null ) {
			alg.showWarning(s+"\n");
		}
	...
	/**
	 * Shows a GRASS processing note.
	 * Notes are never logged and are used for stuff like the
	 * topology builder's output.
	 * 
	 * @param s
	 * 			message string
	 * @param alg
	 * 			GrassAlgorithm that emitted the message
	 */
	public static void addNote(final String s, GrassAlgorithm alg) {
		if ( s != null && alg != null ) {
			alg.showNote(s+"\n");
		}
	}
