...
/**
 * A dialog for selecting multiples elements (raster layers, vector layers or
 * tables)
 *
 * @author volaya
 *
...
public class MultipleInputSelectionDialog extends JDialog {
     ...
     *            a list the indices of the values to show as selected when
     *            opening this dialog. If the user selects ok, the new selection
     *            will be stored in this arraylist, clearing the previous
     *            selection. If not, it will be left unchanged
     */
    @SuppressWarnings("rawtypes")
    public MultipleInputSelectionDialog(final Frame window,
            final Object[] values, final ArrayList selectedIndices) {
        ...
        try {
            ...
            }
            {
                ...
                }
                {
                    ...
                    jButtonDeselectAll
                            .setText(Sextante.getText("Deselect_all"));
    ...
    protected void selectAll(final boolean bSelect) {
        ...
        for (i = 0; i < jList.getModel().getSize(); i++) {
            final JCheckBox checkBox = (JCheckBox) jList.getModel()
                    .getElementAt(i);
