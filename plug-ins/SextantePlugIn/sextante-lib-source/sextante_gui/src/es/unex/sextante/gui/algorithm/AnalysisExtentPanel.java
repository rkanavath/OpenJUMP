...
public class AnalysisExtentPanel extends javax.swing.JPanel {
	...
	private void initGUI() {
		...
		final TableLayout jPanelParametersRasterLayout = new TableLayout(new double[][] 
				{ { 1.0, TableLayoutConstants.FILL, 1.0 }, //Cols
			{ 1.0, TableLayoutConstants.MINIMUM, TableLayoutConstants.FILL, 1.0 } }); //Rows
	...
	public void assignExtent() throws TooLargeGridExtentException, WrongAnalysisExtentException {
		...
		/* benducke: let's just assume the user knows what (s)he is doing...
	...
	protected JPanel getJPanelAnalysisExtentValues() {
		...
		if (jPanelAnalysisExtentValues == null) {
			...
			final TableLayout jPanelRasterExtentValuesLayout = new TableLayout(new double[][] {
				{ 255.0, TableLayoutConstants.FILL, TableLayoutConstants.FILL }, //Cols
				{ 	//Rows
					TableLayoutConstants.MINIMUM, //0: X range 
					TableLayoutConstants.MINIMUM, //1: Y range
					TableLayoutConstants.MINIMUM, //2: Cell size
					TableLayoutConstants.MINIMUM, //3: Cell size picker
					TableLayoutConstants.MINIMUM, //4: Cols/rows count
			...
			jPanelAnalysisExtentValues.add(getJLabelCellSizeXY(), 	"0, 3");
			jPanelAnalysisExtentValues.add(getJTextFieldCellSizeX(),"1, 3");									
			jPanelAnalysisExtentValues.add(getJTextFieldCellSizeY(),"2, 3");			
			jPanelAnalysisExtentValues.add(getJLabelRowsCols(), 	"0, 4");
			jPanelAnalysisExtentValues.add(getJTextFieldCols(), 	"1, 4");
			jPanelAnalysisExtentValues.add(getJTextFieldRows(), 	"2, 4");
		}
	...
	protected JButton getJButtonExtentFromPoints() {
		if ( jButtonExtentFromPoints == null ) {
			...
			jButtonExtentFromPoints.addActionListener(new ActionListener() {
				public void actionPerformed(final ActionEvent arg0) {
					...
					if (ret != null) {	
						getJTextFieldXMin().setText(new DecimalFormat(COORD_FORMAT).format(ret[0]));            	
						getJTextFieldYMin().setText(new DecimalFormat(COORD_FORMAT).format(ret[1]));
						getJTextFieldXMax().setText(new DecimalFormat(COORD_FORMAT).format(ret[2]));
						getJTextFieldYMax().setText(new DecimalFormat(COORD_FORMAT).format(ret[3]));
						extentHasChanged();
					}
				}
			});
			...
			jComboBoxLayers.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(final java.awt.event.ActionEvent evt) {
					...
					try {
						...
						getJTextFieldXMin().setText(new DecimalFormat(COORD_FORMAT).format(dCoord));
						dCoord = layer.getFullExtent().getMaxX();
						getJTextFieldXMax().setText(new DecimalFormat(COORD_FORMAT).format(dCoord));
						dCoord = layer.getFullExtent().getMinY();
						getJTextFieldYMin().setText(new DecimalFormat(COORD_FORMAT).format(dCoord));
						dCoord = layer.getFullExtent().getMaxY();
						getJTextFieldYMax().setText(new DecimalFormat(COORD_FORMAT).format(dCoord));
	...
	private JComboBox getJComboBoxViews() {
		if (jComboBoxViews == null) {
			final ComboBoxModel jComboBoxViewsModel = new DefaultComboBoxModel(SextanteGUI.getInputFactory().
			...
			jComboBoxViews.addActionListener(new java.awt.event.ActionListener() {
				public void actionPerformed(final java.awt.event.ActionEvent evt) {
					...
					try {
						...
						if (ne != null) {
							Rectangle2D.Double extent = ne.getExtent();
							if ( extent == null ) {
	...
	protected JTextField getJTextFieldXMin() {
		if (jTextFieldXMin == null) {
			...
			jTextFieldXMin.addMouseListener(new TextFieldServices(jTextFieldXMin) {});
		}
	...
	protected JTextField getJTextFieldYMin() {
		if (jTextFieldYMin == null) {
			...
			jTextFieldYMin.addMouseListener(new TextFieldServices(jTextFieldYMin) {});
		}
	...
	protected JTextField getJTextFieldYMax() {
		if (jTextFieldYMax == null) {
			...
			jTextFieldYMax.addMouseListener(new TextFieldServices(jTextFieldYMax) {});
		}
	...
	public double getXMin() {
		...
		try {
			if ( Double.valueOf(this.getJTextFieldXMin().getText()) != null ) {
				return (Double.parseDouble(this.getJTextFieldXMin().getText()));
			}
	...
	public double getXMax() {
		...
		try {
			if ( Double.valueOf(this.getJTextFieldXMax().getText()) != null ) {
				return (Double.parseDouble(this.getJTextFieldXMax().getText()));
			}
		} catch ( NumberFormatException nfe ) {
			return (0.0);
		}
	...
	public double getYMin() {
		...
		try {
			if ( Double.valueOf(this.getJTextFieldYMin().getText()) != null ) {
				return (Double.parseDouble(this.getJTextFieldYMin().getText()));
			}
		} catch ( NumberFormatException nfe ) {
			return (0.0);
		}
	...
	public double getYMax() {
		...
		try {
			if ( Double.valueOf(this.getJTextFieldYMax().getText()) != null ) {
				return (Double.parseDouble(this.getJTextFieldYMax().getText()));
			}
		} catch ( NumberFormatException nfe ) {
			return (0.0);
		}
	...
	protected JTextField getJTextFieldCellSizeX() {
		if (jTextFieldCellSizeX == null) {
			...
			jTextFieldCellSizeX.addMouseListener(new TextFieldServices(jTextFieldCellSizeX, this) {});
	...
	protected JTextField getJTextFieldCellSizeY() {
		if (jTextFieldCellSizeY == null) {
			...
			jTextFieldCellSizeY.addMouseListener(new TextFieldServices(jTextFieldCellSizeY, this) {});
	...
	protected void extentHasChanged() {
		...
		try {
			...
			//=599.72932330827
			dCellSizeX = Double.parseDouble(getJTextFieldCellSizeX().getText());
			//=1.5338345864661653
			dCellSizeY = Double.parseDouble(getJTextFieldCellSizeY().getText());
			//=1.5338345864661653
	...
	public void setExtent(final AnalysisExtent ge) {
		...
		getJTextFieldXMin().setText(new DecimalFormat(COORD_FORMAT).format(ge.getXMin()));
		getJTextFieldXMax().setText(new DecimalFormat(COORD_FORMAT).format(ge.getXMax()));
		getJTextFieldYMin().setText(new DecimalFormat(COORD_FORMAT).format(ge.getYMin()));
		getJTextFieldYMax().setText(new DecimalFormat(COORD_FORMAT).format(ge.getYMax()));
		extentHasChanged();
	}
