		...
		for (int j = 0; j < 2; j++) { //twice to avoid errors with dependencies
			for (i = 0; i < alg.getNumberOfParameters(); i++) {
				...
				if (!setParameter(alg, param, sValue) && (j == 1)) {
					throw new CommandLineException("Invalid or missing parameter: '" + param.getParameterName() +"'.");
				}
			}
			...
			if (param instanceof ParameterDataObject) {
				if (!sValue.equals("#")) {
					...
					if (obj != null) {
						...
						else if (param instanceof ParameterImageLayer) {
							param.setParameterValue(obj);
						}
				...
				if (parent instanceof ParameterRasterLayer || parent instanceof ParameterImageLayer) {
