...
/**
 * A panel showing the list of available algorithms, from which they can be
 * executed
 * 
 * @author volaya
 * 
...
public class AlgorithmsPanel extends JPanel {
	 ...
	 * 
	 */
    private static final long serialVersionUID = -5456903267377839108L;
     ...
     * 
     * @param parentDialog
     *            the parent dialog. It will be the parent dialog of parameters
     *            dialog opened from this panel
     * @param filter
    ...
    private void updateSelectedAlgorithm() {
        ...
        if (m_Path != null) {
            ...
            final IToolboxRightButtonAction[] actions = SextanteGUI
                    .getToolboxRightButtonActions();
            for (int i = 0; i < menuItemToolboxAction.length; i++) {
                menuItemToolboxAction[i].setVisible(actions[i]
                        .canBeExecutedOnAlgorithm(m_Alg));
            }

            if (bMeets && m_Alg.requiresIndividualVectorLayers()) {
                m_iExecuteIterativeMenuCount = 0;
                final int iParams = m_Alg.getParameters()
                        .getNumberOfParameters();
                for (int i = 0; i < iParams; i++) {
                    final Parameter param = m_Alg.getParameters().getParameter(
                            i);
                    if (param instanceof ParameterVectorLayer) {
                        final ParameterVectorLayer pvl = (ParameterVectorLayer) param;
                        boolean bMandatory;
                        try {
                            bMandatory = ((AdditionalInfoVectorLayer) pvl
                                    .getParameterAdditionalInfo())
                                    .getIsMandatory();
                            if (bMandatory) {
                                menuItemExecuteIterative[m_iExecuteIterativeMenuCount]
                                        .setText(Sextante
                                                .getText("Run_iterative")
                                                + " ("
    ...
    private void init() {
        ...
        final MouseListener ml = new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                ...
                // Create again DataObjects here to get feature selections
        ...
        menuItemExecuteAsBatch = new JMenuItem(
                Sextante.getText("Execute_as_batch_process"));
        ...
        menuItemExecuteAsBatchFromGIS = new JMenuItem(
                Sextante.getText("Execute_as_batch_process__using_layers_from_GIS_app"));
        ...
        menuItemSetOutputRendering = new JMenuItem(
                Sextante.getText("set_output_rendering"));
        ...
        final JMenuItem menuItemExpand = new JMenuItem(
                Sextante.getText("Expand_all"));
        ...
        final JMenuItem menuItemCollapse = new JMenuItem(
                Sextante.getText("Collapse_all"));
        ...
        menuItemShowOnlyActive = new JCheckBoxMenuItem(
                Sextante.getText("Show_active_only"));
        ...
        menuItemShowOnlyActive.setSelected(SextanteGUI
    ...
    protected void setOutputRendering() {
        ...
        if (m_Alg != null) {
            final OutputRenderingSettingsDialog dialog = new OutputRenderingSettingsDialog(
                    m_Alg);
            dialog.setVisible(true);
            final HashMap<String, Object> set = dialog.getSettings();
            if (set != null) {
                SextanteGUI.getDataRenderer().setRenderingForAlgorithm(
                        m_Alg.getCommandLineName(), set);
    ...
    protected void executeAsBatch() {
        ...
        if (m_Alg != null) {
            SextanteGUI.getGUIFactory().showBatchProcessingDialog(m_Alg,
                    m_ParentDialog.getDialog());
        }
    ...
    protected void executeAsBatchInGIS() {
        ...
        if (m_Alg != null) {
            SextanteGUI.getGUIFactory().showBatchProcessingFromGISDialog(m_Alg,
                    m_ParentDialog.getDialog());
        }
        ...
        try {
            if (m_Alg != null) {
                ...
                final int iRet = SextanteGUI.getGUIFactory()
                        .showAlgorithmDialog(alg, m_ParentDialog.getDialog(),
                                null);
                if (iRet == IGUIFactory.OK) {
                    ...
                    GeoAlgorithmExecutors.execute(alg,
                            m_ParentDialog.getDialog());
    ...
    private void updateListOfMostRecentAlgorithms() {
        ...
        if (bShowMostRecent) {
            ...
            final DefaultMutableTreeNode recentNode = new DefaultMutableTreeNode(
                    Sextante.getText("RecentAlgorithms"));
            for (int i = 0; i < recent.length; i++) {
                final DefaultMutableTreeNode node = new DefaultMutableTreeNode(
                        recent[i]);
                recentNode.add(node);
            }
            final DefaultMutableTreeNode mainNode = (DefaultMutableTreeNode) new TreePath(
                    jTree.getModel().getRoot()).getLastPathComponent();
            mainNode.remove(0);
    ...
    protected void executeSelectedAlgorithmIteratively(
            final int iParameterToIterateOver) {
            ...
            if (m_Alg != null) {
                ...
                if (iRet == IGUIFactory.OK) {
                    ...
                    for (int i = 0; i < iParamCount; i++) {
                        final Parameter param = m_Alg.getParameters()
                                .getParameter(i);
                        if (param instanceof ParameterVectorLayer) {
                            if (iVectorLayers == iParameterToIterateOver) {
                                GeoAlgorithmExecutors.executeIterative(alg,
                                        m_ParentDialog.getDialog(),
     ...
     *            true if it should search in help files. if false, it will only
    ...
    public int fillTree(final String sSearchString,
            ...
            while (iter.hasNext()) {
                ...
                if (m_Filter.accept(alg)) {
                    if (bSearchInHelpFiles) {
                        if (!HelpIO
                                .containsStringInHelpFile(alg, sSearchString)) {
                            continue;
                        }
                    } else {
                        if ((sSearchString != null)
                                && !alg.getName().toLowerCase()
                                        .contains(sSearchString)) {
                            continue;
                        }
                    }
                    iCount++;
                    final AlgorithmGroupConfiguration conf = AlgorithmGroupsOrganizer
                            .getGroupConfiguration(alg);
                    if (conf != null) {
                        if (!conf.isShow()) {
                            continue;
                        }
                    ...
                    HashMap<String, HashMap<String, DefaultMutableTreeNode>> group = groups
                            .get(sGroup);
                    if (group == null) {
                        group = new HashMap<String, HashMap<String, DefaultMutableTreeNode>>();
                        groups.put(sGroup, group);
                    }
                    HashMap<String, DefaultMutableTreeNode> subgroup = group
                            .get(sSubgroup);
                    if (subgroup == null) {
        ...
        // toolbox actions
        final HashMap<NameAndIcon, ArrayList<ToolboxAction>> allActions = SextanteGUI
                .getToolboxActions();
        ...
        while (actionsIter.hasNext()) {
            ...
            for (int i = 0; i < actions.size(); i++) {
                ...
                if ((sSearchString != null)
                        && !ita.getName().toLowerCase().contains(sSearchString)) {
                    continue;
                }
                iCount++;
        ...
        while (iter.hasNext()) {
            ...
            final HashMap<String, HashMap<String, DefaultMutableTreeNode>> g = groups
                    .get(key);
