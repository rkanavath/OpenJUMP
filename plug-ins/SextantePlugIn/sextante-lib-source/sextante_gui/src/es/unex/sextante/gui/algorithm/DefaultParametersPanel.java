 ...
 * based on the parameters needed by that geoalgorithm If no additional panel is
...
public class DefaultParametersPanel extends GeoAlgorithmParametersPanel {
    ...
    protected void initGUI() {
...

        {
            ...
            if (inputs_matrix[1].length > 1) {
                ...
                jInputs.setBorder(BorderFactory.createTitledBorder(Sextante
                        .getText("Inputs")));
            ...
            if (options_matrix[1].length > 1) {
                final TableLayout jOptionsLayout = new TableLayout(
                        options_matrix);
                jOptionsLayout.setHGap(5);
                jOptionsLayout.setVGap(5);
                jOptions.setLayout(jOptionsLayout);

                jOptions.setBorder(BorderFactory.createTitledBorder(Sextante
                        .getText("Options")));
            ...
            if (outputs_matrix[1].length > 1) {
                ...
                final TableLayout jOutputsLayout = new TableLayout(
                        getOutputsTableLayoutMatrix());
                jOutputsLayout.setHGap(5);
                jOutputsLayout.setVGap(5);
                jOutputs.setLayout(jOutputsLayout);

                jOutputs.setBorder(BorderFactory.createTitledBorder(Sextante
                        .getText("Outputs")));
            ...
            if (m_Algorithm.requiresRasterLayers()
                    || m_Algorithm.requiresMultipleRasterLayers()
                    || m_Algorithm.requiresMultipleRasterBands()) {
                addRasterLayers(jInputs);
            }
            if (m_Algorithm.requiresImageLayers()) {
                addImageLayers(jInputs);
            }
            if (m_Algorithm.requires3DRasterLayers()) {
                add3DRasterLayers(jInputs);
            }
            if (m_Algorithm.requiresVectorLayers()
                    || m_Algorithm.requiresMultipleVectorLayers()) {
                addVectorLayers(jInputs);
            }
            if (m_Algorithm.requiresTables()
                    || m_Algorithm.requiresMultipleTables()) {
                addTables(jInputs);
            }
            ...
            jScrollPanelParameters
                    .setHorizontalScrollBarPolicy(ScrollPaneConstants.HORIZONTAL_SCROLLBAR_NEVER);
            jScrollPanelParameters
                    .setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
            jScrollPanelParameters.setPreferredSize(new java.awt.Dimension(680,
                    300));
            ...
            for (int i = 0; i < m_ParameterContainer.size(); i++) {
                final ParameterContainer pc = (ParameterContainer) m_ParameterContainer
                        .get(i);
                final HelpElement help = (HelpElement) m_HelpElements.get(pc
                        .getParameter().getParameterName());
                if (m_Algorithm instanceof GrassAlgorithm) {
                    if (pc.getParameter().getParameterTooltip() != null) {
                        pc.getContainer().setToolTipText(
                                pc.getParameter().getParameterTooltip());
                    }
                }
                if (help != null) {
                    pc.getContainer().setToolTipText(
                            "<html>" + help.getTextAsFormattedHTML()
                                    + "</html>");
                }
            }
    ...
    protected void addOutputObjects(final JPanel pane) {
        ...
        for (int i = 0; i < ooset.getOutputObjectsCount(); i++) {
            ...
            if ((out instanceof OutputRasterLayer)
                ...
                final HelpElement help = (HelpElement) m_HelpElements.get(out
                        .getName());
                final OutputChannelSelectionPanel panel = new OutputChannelSelectionPanel(
                        out, m_Algorithm.getParameters());

                if (Sextante.getAlgorithmProviderName(m_Algorithm).equals(
                        "SEXTANTE")
                        && help != null) {
                    panel.setToolTipText("<html>"
                            + help.getTextAsFormattedHTML() + "</html>");
                } else {
                    if (Sextante.getAlgorithmProviderName(m_Algorithm).equals(
                            "SEXTANTE") == false) {
                        /*
                         * This is a non-native algorithm provider: use
                         * out.getName() to label the browser button and
                         * out.getDescription() to provide a tooltip.
                         */
                        sDescription = out.getName() + "["
                                + out.getTypeDescription() + "]";
                        panel.setToolTipText(out.getDescription());
                    }
                }
                addTitleLabel(pane, sDescription, m_iCurrentRow, false, false);
    ...
    protected void addRasterLayers(final JPanel pane) {
        ...
        addTitleLabel(pane, Sextante.getText("Raster_layers"), m_iCurrentRow,
                false, true);
        m_iCurrentRow++;
        ...
        for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
            ...
            if (parameter instanceof ParameterRasterLayer) {
                ...
                try {
                    isMandatory = (((AdditionalInfoRasterLayer) parameter
                            .getParameterAdditionalInfo()).getIsMandatory());
                ...
                for (j = 0; j < m_Algorithm.getNumberOfParameters(); j++) {
                    ...
                    if (subParameter instanceof ParameterBand) {
                        try {
                            if (((AdditionalInfoBand) subParameter
                                    .getParameterAdditionalInfo())
                                    .getParentParameterName().equals(
                                            sParameterName)) {
                                m_iCurrentRow++;
                                addTitleLabel(
                                        pane,
                                        ...
                                        m_iCurrentRow, isMandatory, false);
                                comboBox = getBandSelectionComboBox();
                                childComboBoxIndex.add(new Integer(m_ComboBox
                                        .size()));
                                m_ComboBox.add(comboBox);
                                pane.add(comboBox,
                                        getStringTableCoords(2, m_iCurrentRow));
                                m_ParameterContainer
                                        .add(new ParameterContainer(
                                                subParameter, comboBox));
                            }
                        } catch (final NullParameterAdditionalInfoException e) {
                            Sextante.addErrorToLog(e);
                        }
                ...
                addTitleLabel(pane, parameter.getParameterDescription(),
                        m_iCurrentRow - childComboBoxIndex.size(), isMandatory,
                        false);
                comboBox = getRasterLayerSelectionComboBox(childComboBoxIndex,
                        bAddNotSetField);
                if (bAddNotSetField) {
                ...
                pane.add(
                        comboBox,
                        getStringTableCoords(2, m_iCurrentRow
                                - childComboBoxIndex.size()));
                m_iCurrentRow++;
                m_ParameterContainer.add(new ParameterContainer(parameter,
                        comboBox));
            } else if (parameter instanceof ParameterMultipleInput) {
                try {
                    final AdditionalInfoMultipleInput additionalInfo = (AdditionalInfoMultipleInput) parameter
                            .getParameterAdditionalInfo();
                    ...
                    if ((additionalInfo.getDataType() == AdditionalInfoMultipleInput.DATA_TYPE_RASTER)
                        ...
                        addMultipleInput(pane,
                                (ParameterMultipleInput) parameter);
                    }
                } catch (final NullParameterAdditionalInfoException e) {
                    Sextante.addErrorToLog(e);
                }
    ...
    protected void addImageLayers(final JPanel pane) {
        ...
        addTitleLabel(pane, Sextante.getText("Image_Layers"), m_iCurrentRow,
                false, true);
        m_iCurrentRow++;

        for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
            parameter = parameters.getParameter(i);
            if (parameter instanceof ParameterImageLayer) {
                boolean isMandatory = false;
                try {
                    isMandatory = (((AdditionalInfoImageLayer) parameter
                            .getParameterAdditionalInfo()).getIsMandatory());
                } catch (final NullParameterAdditionalInfoException e) {
                    Sextante.addErrorToLog(e);
                }
                ...
                addTitleLabel(pane, parameter.getParameterDescription(),
                        m_iCurrentRow - childComboBoxIndex.size(), isMandatory,
                        false);
                comboBox = getImageLayerSelectionComboBox(childComboBoxIndex,
                        bAddNotSetField);
                if (bAddNotSetField) {
                    comboBox.setSelectedIndex(0);
                }
    ...
    protected void add3DRasterLayers(final JPanel pane) {
        ...
        addTitleLabel(pane, Sextante.getText("3DRaster_layers"), m_iCurrentRow,
                false, true);
        m_iCurrentRow++;
        ...
        for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
            ...
            if (parameter instanceof Parameter3DRasterLayer) {
                boolean isMandatory = false;
                try {
                    isMandatory = (((AdditionalInfo3DRasterLayer) parameter
                            .getParameterAdditionalInfo()).getIsMandatory());
                } catch (final NullParameterAdditionalInfoException e) {
                    Sextante.addErrorToLog(e);
                }
    ...
    protected void addVectorLayers(final JPanel pane) {
        ...
        addTitleLabel(pane, Sextante.getText("Vector_layer"), m_iCurrentRow,
                false, true);
        m_iCurrentRow++;
        ...
        for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
            ...
            if (parameter instanceof ParameterVectorLayer) {
                boolean isMandatory = false;
                try {
                    isMandatory = (((AdditionalInfoVectorLayer) parameter
                            .getParameterAdditionalInfo()).getIsMandatory());
                    iShapeType = ((AdditionalInfoVectorLayer) parameter
                            .getParameterAdditionalInfo()).getShapeType();
                    ...
                    for (j = 0; j < m_Algorithm.getNumberOfParameters(); j++) {
                        ...
                        if (subParameter instanceof ParameterTableField) {
                            try {
                                if (((AdditionalInfoTableField) subParameter
                                        .getParameterAdditionalInfo())
                                        .getParentParameterName().equals(
                                                sParameterName)) {
                                    m_iCurrentRow++;
                                    ...
                                    childComboBoxIndex.add(new Integer(
                                            m_ComboBox.size()));
                                    m_ComboBox.add(comboBox);
                                    pane.add(
                                            comboBox,
                                            getStringTableCoords(2,
                                                    m_iCurrentRow));
                                    m_ParameterContainer
                                            .add(new ParameterContainer(
                                                    subParameter, comboBox));
                ...
                try {
                    ...
                    addTitleLabel(pane, parameter.getParameterDescription(),
                            m_iCurrentRow - childComboBoxIndex.size(),
                            isMandatory, false);
                    comboBox = getVectorLayerSelectionComboBox(iShapeType,
                            childComboBoxIndex, bAddNotSetField);
                    if (bAddNotSetField) {
                        comboBox.setSelectedIndex(0);
                    }
                    pane.add(
    ...
    protected void addTables(final JPanel pane) {
        ...
        addTitleLabel(pane, Sextante.getText("Tables"), m_iCurrentRow, false,
                true);
        m_iCurrentRow++;
        ...
        for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
            ...
            if (parameter instanceof ParameterTable) {
                boolean isMandatory = false;
                try {
                    isMandatory = (((AdditionalInfoTable) parameter
                            .getParameterAdditionalInfo()).getIsMandatory());
                } catch (final NullParameterAdditionalInfoException e) {
                    Sextante.addErrorToLog(e);
                }
                ...
                addTitleLabel(pane, parameter.getParameterDescription(),
                        m_iCurrentRow - childComboBoxIndex.size(), isMandatory,
                        false);
                comboBox = getTableSelectionComboBox(childComboBoxIndex,
                        bAddNotSetField);
                if (bAddNotSetField) {
    ...
    protected void addMultipleInput(final JPanel pane,
        ...
        try {
            ...
            case AdditionalInfoMultipleInput.DATA_TYPE_VECTOR_ANY:
                objs = SextanteGUI.getInputFactory().getVectorLayers(
            ...
            if (!additionalInfo.getIsMandatory()) {
                ...
                addTitleLabel(pane, sDescription, m_iCurrentRow, true, false);
            }

            final CheckComboBox ccb = new CheckComboBox();
            ccb.setTextFor(CheckComboBox.NONE,
                    Sextante.getText("no_elements_selected"));
            ccb.setTextFor(CheckComboBox.MULTIPLE,
                    Sextante.getText("multiple_elements_selected"));
            final ListCheckModel model = ccb.getModel();
            for (final Object obj : objs) {
                model.addElement(obj);
            }
            final BatchSelection bs = new BatchSelection.CheckBox();
            final EmbeddedComponent comp = new EmbeddedComponent(bs,
                    Anchor.NORTH);
            ccb.setEmbeddedComponent(comp);

            addTitleLabel(pane, parameter.getParameterDescription(),
                    m_iCurrentRow, additionalInfo.getIsMandatory(), false);
    ...
    protected void addFixedTable(final JPanel pane,
        ...
        try {
            final AdditionalInfoFixedTable additionalInfo = (AdditionalInfoFixedTable) parameter
                    .getParameterAdditionalInfo();
            ...
            addTitleLabel(pane, parameter.getParameterDescription(),
                    m_iCurrentRow, false, false);

            final FixedTablePanel fixedTablePanel = new FixedTablePanel(sCols,
                    iRows, bIsNumberOfRowsFixed);
            addTitleLabel(pane, parameter.getParameterDescription(),
                    m_iCurrentRow, false, false);
    ...
    protected void addFilepath(final JPanel pane,
        ...
        try {
            final AdditionalInfoFilepath additionalInfo = (AdditionalInfoFilepath) parameter
                    .getParameterAdditionalInfo();
            ...
            final FileSelectionPanel fileSelectionPanel = new FileSelectionPanel(
                    additionalInfo.isFolder(), additionalInfo.isOpenDialog(),
                    additionalInfo.getExtensions(), Sextante.getText("Files")
                            + " " + sExtension);
            addTitleLabel(pane, parameter.getParameterDescription(),
                    m_iCurrentRow, false, false);

            fileSelectionPanel.getTextField().addMouseListener(
                    new TextFieldServices(fileSelectionPanel.getTextField()) {
                    });
    ...
    protected void addPoint(final JPanel pane, final ParameterPoint parameter) {

        addTitleLabel(pane, parameter.getParameterDescription(), m_iCurrentRow,
                false, false);
    ...
    protected void addSelection(final JPanel pane,
        ...
        try {
            ...
            final String[] sValues = ((AdditionalInfoSelection) parameter
                    .getParameterAdditionalInfo()).getValues();
            final DefaultComboBoxModel defaultModel = new DefaultComboBoxModel(
                    sValues);
            ...
            m_iCurrentRow++;
            m_ParameterContainer
                    .add(new ParameterContainer(parameter, comboBox));
        } catch (final NullParameterAdditionalInfoException e) {
            Sextante.addErrorToLog(e);
        }
    ...
    protected void addNumericalTextField(final JPanel pane,
        ...
        try {
            final AdditionalInfoNumericalValue additionalInfo = (AdditionalInfoNumericalValue) parameter
                    .getParameterAdditionalInfo();
            ...
            textField.addMouseListener(new TextFieldServices(textField) {
            });

            if (iType == AdditionalInfoNumericalValue.NUMERICAL_VALUE_DOUBLE) {
                textField.setText(Double.toString(additionalInfo
                        .getDefaultValue()));
            } else {
                textField.setText(Integer.toString((int) additionalInfo
                        .getDefaultValue()));
            }
                            ...
                            if (iType == AdditionalInfoNumericalValue.NUMERICAL_VALUE_DOUBLE) {
                                ...
                                if (i > dMaxValue) {
                                    textField.setText(Integer
                                            .toString((int) dMaxValue));
                                }
                                if (i < dMinValue) {
                                    textField.setText(Integer
                                            .toString((int) dMinValue));
                                }
    ...
    protected void addStringTextField(final JPanel pane,
        ...
        try {
            final AdditionalInfoString ai = (AdditionalInfoString) parameter
                    .getParameterAdditionalInfo();
    ...
    protected void addCheckBox(final JPanel pane,
        ...
        final AdditionalInfoBoolean ai = (AdditionalInfoBoolean) parameter
                .getParameterAdditionalInfoOrNull();
        if (ai == null) {
            /*
             * We have no additional info for this boolean. It might have been
             * converted to a String param. Use the parameter's "selected" flag
             * instead.
             */
            checkBox.setSelected(parameter.getParameterSelected());
        } else {
    ...
    protected void addTitleLabel(final JPanel pane, final String sText,
            final int iRow, final boolean isMandatory,
    ...
    protected String getStringTableCoords(final int iCol, final int iRow) {
    ...
    protected double[][] getInputsTableLayoutMatrix() {
        ...
        if (m_Algorithm.requiresImageLayers()) {
            // Adds 1 for label
            iRows += 1;
            iRows += m_Algorithm.getNumberOfImageLayers();
        }
        if (m_Algorithm.requiresVectorLayers()
                || m_Algorithm.requiresMultipleVectorLayers()) {
            // Adds 1 for label
    ...
    protected JComboBox getRasterLayerSelectionComboBox(
            final ArrayList childComboBoxes, final boolean bAddNotSetField) {
        ...
        final IRasterLayer[] layers = SextanteGUI.getInputFactory()
                .getRasterLayers();
        ...
        if (bAddNotSetField) {
            final DefaultComboBoxModel model = (DefaultComboBoxModel) cb
                    .getModel();
            model.insertElementAt(
                    new ObjectAndDescription(
                            Sextante.getText("[Not_selected]"), null), 0);
        }
        ...
        cb.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(final java.awt.event.ItemEvent e) {
                ...
                for (i = 0; i < childsArray.length; i++) {
                    ...
                    final ObjectAndDescription ob = (ObjectAndDescription) cb
                            .getSelectedItem();
                    ...
                    if (layer != null) {
                        ...
                        if (sNames != null) {
                            ...
                            ((JComboBox) m_ComboBox.get(iIndex))
    ...
    protected JComboBox getImageLayerSelectionComboBox(
            final ArrayList childComboBoxes, final boolean bAddNotSetField) {
    ...
    protected JComboBox get3DRasterLayerSelectionComboBox(
            final boolean bAddNotSetField) {

        int i;

        final I3DRasterLayer[] layers = SextanteGUI.getInputFactory()
                .get3DRasterLayers();
        final ObjectAndDescription[] oad = new ObjectAndDescription[layers.length];
        for (i = 0; i < layers.length; i++) {
            oad[i] = new ObjectAndDescription(layers[i].getName(), layers[i]);
    ...
    protected JComboBox getVectorLayerSelectionComboBox(final int iShapeType,
        ...
        final IVectorLayer[] layers = SextanteGUI.getInputFactory()
                .getVectorLayers(iShapeType);
        final ObjectAndDescription[] oad = new ObjectAndDescription[layers.length];
        for (i = 0; i < layers.length; i++) {
            oad[i] = new ObjectAndDescription(layers[i].getName(), layers[i]);
        }
    ...
    protected JComboBox getTableSelectionComboBox(
            final ArrayList childComboBoxes, final boolean bAddNotSetField) {
        ...
        comboBox.addItemListener(new java.awt.event.ItemListener() {
            public void itemStateChanged(final java.awt.event.ItemEvent e) {
                ...
                for (i = 0; i < childsArray.length; i++) {
                    iIndex = (childsArray[i]).intValue();
                    final ObjectAndDescription ob = (ObjectAndDescription) comboBox
                            .getSelectedItem();
    ...
    protected JComboBox getVectorLayerFieldSelectionComboBox(
        ...
        if (layers.length > 0) {
            final JComboBox comboBox = new JComboBox();
            final DefaultComboBoxModel defaultModel = new DefaultComboBoxModel(
                    getVectorLayerFieldNames(layers[0]));
            comboBox.setModel(defaultModel);
            return comboBox;
        } else {
            return new JComboBox();
    ...
    public void assignParameters() throws WrongInputException,
        ...
        for (i = 0; i < m_ParameterContainer.size(); i++) {
            parameterContainer = (ParameterContainer) m_ParameterContainer
                    .get(i);
            ...
            if (sType.equals("Table")) {
                bAssigningOK = assignInputTable(parameterContainer);
                    ...
                    || sType.equals("Image Layer")
                    || sType.equals("Raster Layer")
                    || sType.equals("3D Raster Layer")) {
            ...
            } else if (sType.equals("Table Field") || sType.equals("Selection")
                    || sType.equals("Band")) {
        ...
        for (i = 0; i < m_OutputParameterContainer.size(); i++) {
            final OutputParameterContainer opc = (OutputParameterContainer) m_OutputParameterContainer
                    .get(i);
            Output out;
            ...
            final OutputChannelSelectionPanel ocsp = (OutputChannelSelectionPanel) opc
                    .getContainer();
    ...
    protected boolean assignInputTable(
            final ParameterContainer parameterContainer) {

        boolean bReturn = true;

        try {
            final JComboBox comboBox = (JComboBox) parameterContainer
                    .getContainer();
            final Parameter parameter = m_Algorithm.getParameters()
                    .getParameter(parameterContainer.getName());
            final ObjectAndDescription oad = (ObjectAndDescription) comboBox
                    .getSelectedItem();
    ...
    protected boolean assignInputLayer(
            final ParameterContainer parameterContainer) {
    ...
    protected boolean assignInputMultipleInput(
            final ParameterContainer parameterContainer) {

        boolean bReturn;

        try {
            // final MultipleInputSelectionPanel panel =
            // (MultipleInputSelectionPanel) parameterContainer.getContainer();
            final CheckComboBox ccb = (CheckComboBox) parameterContainer
                    .getContainer();
            final ParameterMultipleInput parameter = (ParameterMultipleInput) m_Algorithm
                    .getParameters().getParameter(parameterContainer.getName());
            final List<Object> list = ccb.getModel().getCheckeds();
            // final ArrayList list = panel.getSelectedObjects();
            final AdditionalInfoMultipleInput ai = (AdditionalInfoMultipleInput) parameter
                    .getParameterAdditionalInfo();
    ...
    protected boolean assignInputSelection(
            final ParameterContainer parameterContainer) {
    ...
    protected boolean assignInputNumericalValue(
            final ParameterContainer parameterContainer) {

        boolean bReturn;

        try {
            final JTextField txtField = (JTextField) parameterContainer
                    .getContainer();
            final Parameter parameter = m_Algorithm.getParameters()
                    .getParameter(parameterContainer.getName());
            bReturn = parameter.setParameterValue(new Double(Double
                    .parseDouble(txtField.getText())));
        } catch (final Exception e) {
            Sextante.addErrorToLog(e);
            return false;
        }
    ...
    protected boolean assignInputFixedTable(
            final ParameterContainer parameterContainer) {

        boolean bReturn;

        try {
            final FixedTablePanel panel = (FixedTablePanel) parameterContainer
                    .getContainer();
            final Parameter parameter = m_Algorithm.getParameters()
                    .getParameter(parameterContainer.getName());
    ...
    protected boolean assignInputString(
            final ParameterContainer parameterContainer) {
    ...
    protected boolean assignInputFilepath(
            final ParameterContainer parameterContainer) {

        boolean bReturn;

        try {
            final FileSelectionPanel fileSelectionPanel = (FileSelectionPanel) parameterContainer
                    .getContainer();
            final Parameter parameter = m_Algorithm.getParameters()
                    .getParameter(parameterContainer.getName());
            bReturn = parameter.setParameterValue(fileSelectionPanel
                    .getFilepath());
        } catch (final WrongParameterIDException e) {
            Sextante.addErrorToLog(e);
            return false;
    ...
    protected boolean assignInputPoint(
            final ParameterContainer parameterContainer) {

        boolean bReturn;

        try {
            final PointSelectionPanel pointSelectionPanel = (PointSelectionPanel) parameterContainer
                    .getContainer();
            final Parameter parameter = m_Algorithm.getParameters()
                    .getParameter(parameterContainer.getName());
            bReturn = parameter.setParameterValue(pointSelectionPanel
                    .getPoint());
        } catch (final Exception e) {
            Sextante.addErrorToLog(e);
            return false;
    ...
    protected boolean assignInputBoolean(
            final ParameterContainer parameterContainer) {

        boolean bReturn;

        try {
            final JCheckBox checkBox = (JCheckBox) parameterContainer
                    .getContainer();
            final Parameter parameter = m_Algorithm.getParameters()
                    .getParameter(parameterContainer.getName());
            parameter.setParameterSelected(new Boolean(checkBox.isSelected()));
            bReturn = parameter.setParameterValue(new Boolean(checkBox
                    .isSelected()));
        } catch (final WrongParameterIDException e) {
            Sextante.addErrorToLog(e);
            return false;
    ...
    @Override
    public void setOutputValue(final String sOutputName, final String sValue) {

        for (int i = 0; i < m_OutputParameterContainer.size(); i++) {
            final OutputParameterContainer container = (OutputParameterContainer) m_OutputParameterContainer
                    .get(i);
            if (sOutputName.equals(container.getName())) {
                final OutputChannelSelectionPanel ocsp = (OutputChannelSelectionPanel) container
                        .getContainer();
                ocsp.setText(sValue);
            }
        }
        ...
        try {
            if (param instanceof ParameterDataObject) {
                ...
                for (int i = 0; i < model.getSize(); i++) {
                    final ObjectAndDescription oad = (ObjectAndDescription) model
                            .getElementAt(i);
                    ...
                    || (param instanceof ParameterString)) {
                if (!sValue.equals("#")) {
                ...
                this.setFixedTableValue(param, (FixedTablePanel) container,
                        sValue);
                ...
                if (!sValue.equals("#")) {
                    ...
                    final AdditionalInfoSelection ai = (AdditionalInfoSelection) param
                            .getParameterAdditionalInfo();
                ...
                // final MultipleInputSelectionPanel msp =
                // (MultipleInputSelectionPanel) container;
                    ...
                    for (int i = 0; i < sObjects.length; i++) {
                        if (objs[j] instanceof IDataObject) {
                            ...
                            try {
                                ...
                                if (rlab.getRasterLayer().getName()
                                        .equals(sObjects[i].trim())) {
                                    if (Integer
                                            .parseInt(sObjects[i + 1].trim()) - 1 == rlab
                                            .getBand()) {
                                        ccb.getModel().addCheck(objs[j]);
                                        // selectedIndices.add(new Integer(j));
                                        break;
                ...
                if (st.countTokens() == 2) {
                    ...
                    || (param instanceof ParameterBand)) {
                final JComboBox cb = (JComboBox) container;
                try {
    ...
    protected void setFixedTableValue(final Parameter param,
            final FixedTablePanel container, final String sValue) {
