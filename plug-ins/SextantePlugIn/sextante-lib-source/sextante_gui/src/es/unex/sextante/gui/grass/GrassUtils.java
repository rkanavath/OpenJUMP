...
import es.unex.sextante.gui.win32.Kernel32;
import es.unex.sextante.gui.win32.W32API;
...
public class GrassUtils {
	...
	//GRASS MESSAGE TYPES
	final public static int GRASS_MSG_TYPE_NONE = 0;
	final public static int GRASS_MSG_TYPE_INFO = 1;
	final public static int GRASS_MSG_TYPE_WARN = 2;
	final public static int GRASS_MSG_TYPE_ERROR = 3;
	final public static int GRASS_MSG_TYPE_NOTE = 4;	
	...
	//Two synchronized lists for mapping real I/O file names to
	//temp GRASS map names:
	private static List<String> m_MapNamesReal = null;
	private static List<String> m_MapNamesTemp = null;
	private static final String m_TokenSeparator = "@";
	...
	public static final String    colorTableIdentifier = "gvSIG CE GRASS Interface Color Table";
	...
	public static File createStartupScript(final String cmdline) {
		...
		tmpBase = GeoAlgorithm.getTempFolder();
		...
		String extraPath = System.getProperty("gvSIG.binPath"); // Check for path extension
		if (Sextante.isUnix() || Sextante.isMacOSX()) {
			...
			try {
				...
				output.write("#!/bin/sh\n");
				output.write("export SEXTANTE_JAVA_GRASS_INTERFACE=\"" + Sextante.getVersionNumber() + "\"\n");
				//Add all temp map to real map name mappings:
				if ( m_MapNamesTemp != null ) {
					String STokenSep = "SEXTANTE_JAVA_GRASS_TOKEN_SEP=\"" + m_TokenSeparator + "\"\n";
					output.write(STokenSep);
					String sMapNames = "export SEXTANTE_JAVA_GRASS_MAP_NAMES=\""; 
					for ( int i=0; i < m_MapNamesTemp.size(); i ++ ) {
						sMapNames += m_MapNamesTemp.get(i);
						sMapNames += m_TokenSeparator;
						sMapNames += m_MapNamesReal.get(i);
						if ( i < m_MapNamesTemp.size()-1 ) {
							sMapNames += ";";
						}
					}
					sMapNames += "\"\n";
					output.write(sMapNames);
				}
				...
				Sextante.getLogger().addError("Error writing start-up script for Linux/Mac OS X: ");
			...
			try {
				...
				output.write("set SEXTANTE_JAVA_GRASS_INTERFACE=" + Sextante.getVersionNumber() + "\n");
				//Add all temp map to real map name mappings:
				if ( m_MapNamesTemp != null ) {
					String STokenSep = "set SEXTANTE_JAVA_GRASS_TOKEN_SEP=" + m_TokenSeparator + "\n";
					output.write(STokenSep);
					String sMapNames = "set SEXTANTE_JAVA_GRASS_MAP_NAMES="; 
					for ( int i=0; i < m_MapNamesTemp.size(); i ++ ) {
						sMapNames += m_MapNamesTemp.get(i);
						sMapNames += m_TokenSeparator;
						sMapNames += m_MapNamesReal.get(i);
				...
				if ( extraPath != null && extraPath.trim() != "" ) {
					output.write("set PATH=" + shToolsPath + ";" + extraPath.trim() + ";" + "%PATH%\n");
				} else {
					output.write("set PATH=" + shToolsPath + ";" + "%PATH%\n");
				}
            ...
            output.write("call \"%GRASS_BATCH_JOB%\" > " + m_ComFile.getAbsolutePath() + " 2>&1\n\n");*/
				...
				Sextante.getLogger().addError("Error writing start-up script for Windows: ");
				Sextante.getLogger().addError(e);
				return (null);
			}
		}
	...
	static ProcessBuilder getGrassExecutable(final String cmdline) {
		...
		if (Sextante.isUnix() || Sextante.isMacOSX()) {
			...
			if (script != null) {
				...
				Sextante.getLogger().addError("Failed to create GRASS start-up script.");
				return null;
			}
		}
		else { //windows
	...
	public static String getBatchJobFile() {
		...
		sFile = Sextante.getUserFolder() + File.separator + sFile;
	 ...
	 */
	public static String getTempMapName(final String sPrefix, final String realName ) {
		String tmpName;
		String sName;
		UUID id;
		boolean exists;
		...
		//Add to lists for real <-> temp name mapping
		if ( m_MapNamesTemp == null ) {
			m_MapNamesTemp = new ArrayList<String>();
		}
		if ( m_MapNamesReal == null ) {
			m_MapNamesReal = new ArrayList<String>();    	  
		}
		//We only store the final part of the input file path.
		if (Sextante.isWindows()) {
			m_MapNamesTemp.add(sName);
			m_MapNamesReal.add(realName.substring(realName.lastIndexOf("\\")+1));
		} else {
			m_MapNamesTemp.add(sName);
			m_MapNamesReal.add(realName.substring(realName.lastIndexOf("/")+1));
		}
	 ...
	 * 
	 */
	public static String getTempMapName(final String realName) {
		return (getTempMapName(GrassUtils.TEMP_PREFIX + "_", realName));
	}
	...
	public static boolean isLatLon() {
		...
		try {
			...
			GrassAlgorithmProvider.addWarning("Could not determine coordinate system. Assuming X/Y(/Z) cartesian.");
		}
		return (false);
	}
	 ...
	 *
	 * Returns the type of output for color-coded message formatting:
	 * 
	 * 	TYPE_NONE = 0;
	 *	TYPE_INFO = 1;
	 *	TYPE_WARN = 2;
	 ...
	 *	TYPE_NOTE = 4;
	 * 
	 * TYPE_NONE will be returned on error.
	 *
	 */
	public static int filterGRASSOutput(String line) throws GrassExecutionException {		

		boolean skip = false;

		int type = GRASS_MSG_TYPE_INFO;		

		//Only process output if we are running GRASS from a SEXTANTE algorithm
		if (m_Alg == null) {
			return (GRASS_MSG_TYPE_NONE);
		}
		...
		if (!line.contains("GRASS_INFO_PERCENT")) {
			...
			if (insideRegPrimitives == true) {
				skip = true;//This is just a simple progress display
			}
			...
			if (line.contains("GRASS_INFO_ERROR") || line.startsWith("ERROR:")) {
				...
				GrassAlgorithmProvider.addError(line, m_Alg);
		...
		if (!skip) {
			...
			{//Log output
				...
				if (!skip) {
					if ( type == GRASS_MSG_TYPE_INFO ) {
						GrassAlgorithmProvider.addInfo(line, m_Alg);
					}
					if ( type == GRASS_MSG_TYPE_WARN ) {
						GrassAlgorithmProvider.addWarning(line, m_Alg);
					}
					if ( type == GRASS_MSG_TYPE_ERROR ) {
						GrassAlgorithmProvider.addError(line, m_Alg);
					}
					if ( type == GRASS_MSG_TYPE_NOTE ) {
						GrassAlgorithmProvider.addNote(line, m_Alg);
					}
				}
			}
		}
	...
	public static void runGRASS(final StringBuffer sCommand,
		...
		//CPU intensive tasks!
		try {
			//Spawn a new process from the JVM 
			final ProcessBuilder pb = GrassUtils.getGrassExecutable(sCommand.toString());
			final Process process = pb.start();
			m_Proc = process;
			//Register the PID of the new process!
			if(process.getClass().getName().equals("java.lang.UNIXProcess")) {
				/* get PID on Unixish systems */
				try {
			...
			//TODO: Don't know how to handle the PID on any other system.
			//TODO: For now, we simply ignore that scenario (the worst effect
			//TODO: would be that cancelling the process won't work).
			if (Sextante.isWindows()) {				
				...
				while (((eisr.read(stderr, pos_stderr, 1)) != -1)) {
				...
				while (((isr.read(stdin, pos_stdin, 1)) != -1)) {
					//Process stdin
	...
	public static void createTempMapset() throws IOException {
		...
		try {
			...
			if (!bIsLatLon) {
				/* XY location */
				out.write("GRASS interface: temporary x/y data processing location.\n");
			}
			else {
				/* lat/lon location */
				out.write("GRASS interface: temporary lat/lon data processing location.\n");
			}
			out.close();
		}
		catch (final IOException e) {
	...
	public static void deleteTempMapset() {
		...
		//Clear list of real file name <-> temp. map mappings.
		m_MapNamesReal = null;
		m_MapNamesTemp = null;
	...
	public static void createTempMapsetName() {
		...
		tmpSuffix = new String("_" + id);
		tmpBase = GeoAlgorithm.getTempFolder();
		Sextante.getLogger().addInfo("GRASS temp files stored in: " + tmpBase);
