...
GeoAlgorithmModelerParametersPanel {
	...
	protected void initGUI() {
		...
		try {
			final ArrayList list = HelpIO.open(SextanteGUI.getAlgorithmHelpFilename(m_Algorithm, false));
		...
		jScrollPanelParameters.setVerticalScrollBarPolicy(ScrollPaneConstants.VERTICAL_SCROLLBAR_AS_NEEDED);
		{
			...
			jPanelParameters.setSize(new java.awt.Dimension(630, 300));
			{
				...
				if (m_Algorithm.requiresImageLayers()) {
					addImageLayers(jPanelParameters);
				}
		...
		try {
			...
			for (int i = 0; i < ooset.getOutputObjectsCount(); i++) {
				...
				if ((out instanceof OutputRasterLayer) || (out instanceof OutputVectorLayer) || (out instanceof OutputTable)
					...
					if (help != null) {
						olsp.getTextField().setToolTipText("<html>" + help.getTextAsFormattedHTML() + "</html>");
					} else {
						/* This is a non-native algorithm provider: use out.getName() to label
						 * the browser button and out.getDescription() to provide a tooltip. */
						sDescription = out.getName();
						olsp.getTextField().setToolTipText(out.getDescription());
					}               
	...
	private void addImageLayers(final JPanel pane) {
		...
		addTitleLabel(pane, Sextante.getText("Image_layers"), m_iCurrentRow, true);
		m_iCurrentRow++;
		...
		for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
			...
			if (parameter instanceof ParameterImageLayer) {
				try {
					if (((AdditionalInfoImageLayer) parameter.getParameterAdditionalInfo()).getIsMandatory()) {
						bIsOptional = false;
						addTitleLabel(pane, parameter.getParameterDescription(), m_iCurrentRow - childComboBoxIndex.size(), false);
					}
					else {
				...
				comboBox = getImageLayerSelectionComboBox(childComboBoxIndex, bIsOptional);
	...
	private void addNonDataObjects(final JPanel pane) {
		...
		for (i = 0; i < m_Algorithm.getNumberOfParameters(); i++) {
			...
			/*
             ...
             addTableField(pane, (ParameterTableField) parameter);
         }
			 */        
	...
	private void addTableField(final JPanel pane,
			final ParameterTableField parameter) {

		JComboBox comboBox;

		try {
			final AdditionalInfoTableField additionalInfo = (AdditionalInfoTableField) parameter.getParameterAdditionalInfo();

			addTitleLabel(pane, parameter.getParameterDescription(), m_iCurrentRow, false);

			comboBox = new JComboBox();
			final ObjectAndDescription values[] = getElementsOfClass(ModelElementTableField.class, true);
	...
	private void addStringTextField(final JPanel pane,
		...
		/*
		 * GRASS algorithms can take Strings, Numbers, point coordinates, Booleans or
		 * attribute field IDs as string type input,
		 * so we allow all of them to be fed into parameters of type String.
		 */         
		final ObjectAndDescription values[] =
				getElementsOfClassAOrBOrCOrDOrE(
						ModelElementString.class, 
						ModelElementNumericalValue.class,
						ModelElementPoint.class,
						ModelElementBoolean.class,
						ModelElementTableField.class,
						true);
	...
	private JComboBox getRasterLayerSelectionComboBox(final ArrayList childComboBoxes,
		...
		final JComboBox comboBox = new JComboBox();
		final ObjectAndDescription[] sObjects = getElementsOfClassAOrB(ModelElementRasterLayer.class, 
				ModelElementImageLayer.class, bIsOptional);
	...
	private JComboBox getImageLayerSelectionComboBox(final ArrayList childComboBoxes,
			final boolean bIsOptional) {

		final JComboBox comboBox = new JComboBox();
		final ObjectAndDescription[] sObjects = getElementsOfClass(ModelElementImageLayer.class, bIsOptional);
		...
		comboBox.addItemListener(new java.awt.event.ItemListener() {
			public void itemStateChanged(final java.awt.event.ItemEvent e) {
				...
				final ObjectAndDescription oad = (ObjectAndDescription) m_DataObjects.get(ob.getObject());
				if (oad != null) {
					final int iBands = ((ModelElementImageLayer) oad.getObject()).getNumberOfBands();
					for (i = 0; i < childsArray.length; i++) {
						iIndex = (childsArray[i]).intValue();
						sNames = getBandNames(iBands);
						if (sNames != null) {
	...
	protected boolean makeDataObjectAssignment(final HashMap map,
		...
		if ( oad == null ) {
			/*
    	  Sextante.addWarningToLog(Sextante.getText("Warn_empty_data_obj"));
		   JOptionPane.showMessageDialog(
				   this,
				   Sextante.getText("Warn_empty_data_obj"),
