...
public class SagaUtils {
   ...
   public static void createLibrariesListFile() throws SagaExecutionException {
      ...
      final String[] sCommands = new String[] { " > " + getSagaDescriptionFolder() + File.separator + "sagalibs.lst" };
   ...
   private static void createLibrariesDescriptionFiles() throws SagaExecutionException {
      ...
      final String sFile = getSagaDescriptionFolder() + File.separator + "sagalibs.lst";
   ...
   public static String getSagaDescriptionFolder() {
      ...
      final String sPath = Sextante.getUserFolder() + File.separator + Sextante.DESC_FOLDER_BASE 
      													+ File.separator + Sextante.DESC_FOLDER_SAGA;
      
      final File base = new File (Sextante.getUserFolder() + Sextante.DESC_FOLDER_BASE);
      if (!base.exists()) {
          base.mkdir();
       }      
   ...
   public static void createSagaBatchJobFileFromSagaCommands(final String[] sCommands) {
      ...
      final String sSagaModsFolder = SextanteGUI.getSettingParameterValue(SextanteSagaSettings.SAGA_MODS_FOLDER);
      try {
         ...
         if (Sextante.isWindows()) {
            ...
            output.write("set SAGA_MLB=" + sSagaModsFolder + "\n");
            output.write("set PATH=%SAGA%;%SAGA_MLB%;%PATH%\n");
         }
         else {
            output.write("#!/bin/sh\n");
            output.write("export SAGA=\"" + sSagaFolder + "\"\n");
            output.write("export SAGA_MLB=\"" + sSagaModsFolder + "\"\n");
            output.write("export PATH=\"" + sSagaFolder + "\":\"$PATH\"\n");
            output.write("export LD_LIBRARY_PATH=\"" + sSagaFolder + "\":\"$LD_LIBRARY_PATH\"\n");
         }
