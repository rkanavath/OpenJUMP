 ...
 * 
 */
public class FixedTableDialog extends javax.swing.JDialog {
     ...
     */
    public FixedTableDialog(final Frame window, final FixedTableModel tableModel) {
        ...
        try {
            {
                this.setTitle(Sextante.getText("Fixed_table"));
                {
                    ...
                    final TableLayout jPanelMainLayout = new TableLayout(
                            new double[][] {
                                    { 3.0, TableLayoutConstants.FILL,
                                            TableLayoutConstants.FILL,
                                            TableLayoutConstants.FILL,
                                            TableLayoutConstants.FILL, 3.0 },
                                    { 3.0, 75.0, TableLayoutConstants.FILL,
                                            75.0, 5.0, 30.0, 3.0 } });
                    jPanelMainLayout.setHGap(5);
                    jPanelMainLayout.setVGap(5);
                    jPanelMain.setLayout(jPanelMainLayout);
                    jPanelMain
                            .setPreferredSize(new java.awt.Dimension(564, 322));
                    {
                        jScrollPane = new JScrollPane();
                        jPanelMain.add(jScrollPane, "1, 1, 3, 3");
                        jScrollPane.setPreferredSize(new java.awt.Dimension(
                                432, 256));
                        {
                            jTable = new JTable();
                    ...
                    }
                    {
                        ...
                        final TableLayout jPanelAddOpenSaveLayout = new TableLayout(
                                new double[][] {
                        ...
                        jPanelOpenSave.setPreferredSize(new java.awt.Dimension(
                                116, 56));
                    }
                    {
                        jPanelAddRemoveRows = new JPanel();
                        final TableLayout jPanelAddRemoveRowsLayout = new TableLayout(
                                new double[][] {
                                        { TableLayoutConstants.MINIMUM,
                                                TableLayoutConstants.FILL,
                                                TableLayoutConstants.MINIMUM },
                        ...
                        jPanelAddRemoveRows
                                .setLayout(jPanelAddRemoveRowsLayout);
                        jPanelMain.add(jPanelAddRemoveRows, "4, 1");
                        jPanelAddRemoveRows
                                .setPreferredSize(new java.awt.Dimension(116,
                                        56));
                    }
                    {
                        ...
                        jButtonRemoveRow
                                .setText(Sextante.getText("Delete_row"));
                        jButtonRemoveRow
                    ...
                    }
                    {
                        ...
                        jSeparator.setPreferredSize(new java.awt.Dimension(565,
                                20));
                    }
                    {
    ...
    private void save() {
        ...
        final GenericFileFilter filter = new GenericFileFilter("csv",
            ...
            try {
                ...
                for (int iRow = 0; iRow < iRows; iRow++) {
                    for (int iCol = 0; iCol < iCols; iCol++) {
                        final String s = m_TableModel.getValueAt(iRow, iCol)
                                .toString();
                        output.write(s);
                        if (iCol < (iCols - 1)) {
                            output.write("|");
    ...
    private void open() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this,
                    Sextante.getText("Could_not_open_selected_file"),
                    Sextante.getText("Error"), JOptionPane.ERROR_MESSAGE);
        }

    }

    private void processLine(final String line, final FixedTableModel table,
            final int iRow) {
    ...
    private void btnOKActionPerformed() {
        ...
        if (checkData()) {
            m_TableModelOrg.setAttributes(m_TableModel.getColumnNames(),
                    m_TableModel.getData(), m_TableModel.isNumberOfRowsFixed());
            ...
            JOptionPane.showMessageDialog(this,
                    Sextante.getText("Wrong_values_in_table"),
                    Sextante.getText("Error"), JOptionPane.ERROR_MESSAGE);
        }
        ...
        try {
            for (int i = 0; i < m_TableModel.getData().length; i++) {
                ...
                for (int j = 0; j < list.size(); j++) {
                    if (list.get(j).toString().isEmpty()) { // just check that
                                                            // every cell
                                                            // contains
                                                            // something
                        return false;
