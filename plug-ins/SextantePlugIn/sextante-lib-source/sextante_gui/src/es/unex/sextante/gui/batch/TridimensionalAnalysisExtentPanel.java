...
public class TridimensionalAnalysisExtentPanel extends AnalysisExtentPanel {
    ...
    @Override
    public void adjustExtent() throws TooLargeGridExtentException,
        ...
        if (!getJRadioButtonAdjustToInputDataExtent().isSelected()) {
            ...
            try {
                m_AnalysisExtent.setCellSizeX(Double
                        .parseDouble(jTextFieldCellSizeX.getText()));
                m_AnalysisExtent.setCellSizeY(Double
                        .parseDouble(jTextFieldCellSizeY.getText()));
                m_AnalysisExtent.setCellSizeZ(Double
                        .parseDouble(jTextFieldCellSizeZ.getText()));
                m_AnalysisExtent.setXRange(
                        Double.parseDouble(jTextFieldXMin.getText()),
                        Double.parseDouble(jTextFieldXMax.getText()), true);
                m_AnalysisExtent.setYRange(
                        Double.parseDouble(jTextFieldYMin.getText()),
                        Double.parseDouble(jTextFieldYMax.getText()), true);
                m_AnalysisExtent.setZRange(
                        Double.parseDouble(jTextFieldZMin.getText()),
            ...
            /*
             * benducke: let's just assume the other knows what (s)he is
             * doing... if (m_AnalysisExtent.getNX() * m_AnalysisExtent.getNY()
             * > BIG_SIZE) { throw new
    ...
    protected JPanel getJPanelAnalysisExtentValues() {
        ...
        if (jPanelAnalysisExtentValues == null) {
            ...
            final TableLayout jPanelRasterExtentValuesLayout = new TableLayout(
                    new double[][] {
                            ...
                            TableLayoutConstants.MINIMUM, // 0: X range
                                    ...
                                    TableLayoutConstants.MINIMUM, // 2: X/Y cell
                                                                  // size
                                    TableLayoutConstants.MINIMUM, // 3:
                                                                  // Cols/rows
                                                                  // count
                                    TableLayoutConstants.MINIMUM, // 4: Z range
                                    TableLayoutConstants.MINIMUM, // 5: Z cell
                                                                  // size
                                    TableLayoutConstants.MINIMUM, // 6: Z levels
                                                                  // count
                                    TableLayoutConstants.FILL } });
            jPanelRasterExtentValuesLayout.setHGap(5);
            jPanelRasterExtentValuesLayout.setVGap(5);
            jPanelAnalysisExtentValues
                    .setLayout(jPanelRasterExtentValuesLayout);
            jPanelAnalysisExtentValues.setBorder(BorderFactory
                    .createTitledBorder(Sextante.getText("Extent__values")));
            jPanelAnalysisExtentValues.setPreferredSize(new java.awt.Dimension(
                    660, 145));
            ...
            jPanelAnalysisExtentValues.add(getJLabelCellSizeXY(), "0, 2");
            jPanelAnalysisExtentValues.add(getJTextFieldCellSizeX(), "1, 2");
            jPanelAnalysisExtentValues.add(getJTextFieldCellSizeY(), "2, 2");
            jPanelAnalysisExtentValues.add(getJLabelRowsCols(), "0, 3");
            jPanelAnalysisExtentValues.add(getJTextFieldCols(), "1, 3");
            jPanelAnalysisExtentValues.add(getJTextFieldRows(), "2, 3");
    ...
    private void extentZHasChanged() {
        ...
        try {
            dRange = Math.abs(Double.parseDouble(getJTextFieldZMax().getText())
                    - Double.parseDouble(getJTextFieldZMin().getText()));
            dCellSize = Double.parseDouble(getJTextFieldCellSizeZ().getText());
            iCols = (int) Math.round(dRange / dCellSize);
