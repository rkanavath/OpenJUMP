...
public class HelpIO {
                ...
                while ((tag != XmlPullParser.END_DOCUMENT) && !bOut) {
                    ...
                    case XmlPullParser.START_TAG:
                        if (parser.getName().compareTo(HELP) == 0) {
                                ...
                                HelpElement.ELEMENT) == 0) {
                            images = new ArrayList();
                            final String sText = parser.getAttributeValue("",
                                    HelpElement.TEXT);
                            ...
                            final int iType = Integer.parseInt(parser
                                    .getAttributeValue("", HelpElement.TYPE));
                            ...
                            element.setDescription(sDescription);
                                ...
                                ImageAndDescription.IMAGE) == 0) {
                            final ImageAndDescription iad = new ImageAndDescription();
                            final String sImageFilename = parser
                                    .getAttributeValue("",
                                            ImageAndDescription.FILE);
                            final String sDesc = parser.getAttributeValue("",
    ...
    /**
     * Returns the help associated with a given geoalgorithm as a html-formatted
     * string
     * 
     * @param alg
     *            the geoalgorithm
    ...
    public static String getHelpAsHTMLCode(final GeoAlgorithm alg,
        ...
        HashMap elements;
        final String sPath = "file:///"
                + sFilename.substring(0, sFilename.lastIndexOf(File.separator))
                + File.separator;
        ...
        if (element != null) {
            ...
            for (int j = 0; j < element.getImages().size(); j++) {
                final ImageAndDescription iad = (ImageAndDescription) element
                        .getImages().get(j);
                doc.addImageAndDescription(sPath + iad.getFilename(),
                        iad.getDescription());
            }
        }
        ...
        for (int i = 0; i < params.getNumberOfParameters(); i++) {
            ...
            sParam = "<b>" + sParam + "[" + getParameterTypeName(param)
                    + "]: </b>";
        ...
        doc.addHeader(Sextante.getText("User_notes"), 2);

        element = (HelpElement) elements.get("USER_NOTES");
        if (element != null) {
            doc.addParagraph(element.getTextAsFormattedHTML());
            for (int j = 0; j < element.getImages().size(); j++) {
                final ImageAndDescription iad = (ImageAndDescription) element
    ...
    private static String getParameterTypeName(final Parameter param) {
        ...
        String s = Sextante.getText(param.getParameterTypeName().replace(' ',
                '_'));

        if (param instanceof ParameterVectorLayer) {
            try {
                final AdditionalInfoVectorLayer ai = (AdditionalInfoVectorLayer) param
    ...
    /**
     * Returns true if the help file associated with an algorithm contains a
     * given search string
     * 
     * @param alg
     *            the GeoAlgorithm
     * @param string
     *            a string to search
     * @return true if the help file associated with the algorithm contains the
     *         search string
     */
    public static boolean containsStringInHelpFile(final GeoAlgorithm alg,
        ...
        try {
            final String sFilename = SextanteGUI.getAlgorithmHelpFilename(alg,
                    false);
     ...
     *            if true, returns the path to the current locale, even if it
     *            does not exist. If false, it will return the path
     *            corresponding to the default locale (english) in case the one
     *            corresponding to the current locale does not exist.
     * 
    ...
    // sFilename =
    // sModelFilename.substring(sModelFilename.lastIndexOf(File.separator));
    // sFilename = sFilename + ".xml";
    // sPath =
    // SextanteGUI.getSettingParameterValue(SextanteModelerSettings.MODELS_FOLDER);
    // }
    ...
    // }
    // sPath = SextanteGUI.getGrassFolder() + File.separator + "docs" +
    // File.separator + "html";
    // }
    // else {
    ...
    public static String getHelpPath(final GeoAlgorithm alg,
        ...
        String sPath = SextanteGUI.getHelpPath() + File.separator
                + Locale.getDefault().getLanguage() + File.separator + sPackage;

        final File dir = new File(sPath);
        if (!dir.exists() && !bForceLocale) {
            sPath = SextanteGUI.getHelpPath() + File.separator
                    + Locale.ENGLISH.getLanguage() + File.separator + sPackage;
        }
    ...
    public static String getHelpFile(final String sTopic) {
        ...
        return sPath + File.separator + "general" + File.separator + sTopic
                + ".html";
