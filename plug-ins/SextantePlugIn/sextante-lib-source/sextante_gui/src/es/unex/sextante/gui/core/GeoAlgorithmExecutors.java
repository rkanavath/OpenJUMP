...
public class GeoAlgorithmExecutors {
   ...
   public static void execute(final GeoAlgorithm alg,
      ...
      final Runnable run = new Runnable() {
         ...
         public void run() {
            ITaskMonitor tm = SextanteGUI.getOutputFactory().getTaskMonitor(alg.getName(), alg.isDeterminatedProcess(), 
            		alg.isSilentProcess(), parent);
            ...
            try {
               ...
               try {
                  SwingUtilities.invokeAndWait(new Runnable() {
                     public void run() {
                        JOptionPane.showMessageDialog(null, Sextante.getText("Error_geoalgorithm_execution"), Sextante.getText("Error"), JOptionPane.ERROR_MESSAGE);
                     }
                  });
               }
               catch (Exception ex) {}
   ...
   public static boolean executeForCommandLine(final GeoAlgorithm alg,
      ...
      final ITaskMonitor tm = SextanteGUI.getOutputFactory().getTaskMonitor(alg.getName(), alg.isDeterminatedProcess(), 
    		  alg.isSilentProcess(),parent);
      final ExecutorService pool = Executors.newFixedThreadPool(1);
