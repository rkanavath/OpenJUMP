...
public class ParameterCellPanel extends JPanel
{
	...
	private static int		FILE_BROWSER_WITH_ACCESSORY_WIDTH = 1024;
	private static int		FILE_BROWSER_WITH_ACCESSORY_HEIGHT = 400;
	...
	private JComboBox getDataObjectComboBox() {
		...
		Object objs[] = null;
		try {
			if (m_Object instanceof ParameterImageLayer) {
				objs = SextanteGUI.getInputFactory().getImageLayers();
			}
			else if (m_Object instanceof ParameterRasterLayer) {
			...
			Sextante.getLogger().addError("Batch processor UI: Error getting list of data objects.");
		}

		final JComboBox combo = new JComboBox(objs);
		textField.setText(combo.getItemAt(0).toString());
	...
	private void btnActionPerformed(final ActionEvent e) {
		...
		else if (m_Object instanceof ParameterFilepath) {
			ParameterFilepath param = (ParameterFilepath) m_Object;
			boolean open = false;
			boolean folder = false;
			try {
				AdditionalInfoFilepath aif = (AdditionalInfoFilepath) param.getParameterAdditionalInfo();
				open = aif.isOpenDialog();
				folder = aif.isFolder();
			} catch (NullParameterAdditionalInfoException e1) {
				Sextante.getLogger().addWarning("No additional info for filepath parameter. Assuming type 'save' and 'no folder'.");
			}			
			assignFilepath(open,folder);
		}
	...
	private void assignFilepath(boolean open, boolean folder) {
		int returnVal;
		final JFileChooser fc = new JFileChooser();

		fc.setPreferredSize(new Dimension(FILE_BROWSER_WIDTH, FILE_BROWSER_HEIGHT));
		
		if ( ParameterCellPanel.LAST_DIR != null ) {
			fc.setCurrentDirectory(new File(ParameterCellPanel.LAST_DIR));
		}
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			// Store this dir as default dir for file browser
			ParameterCellPanel.LAST_DIR = new File (sFile).getParent();

			m_Table.getModel().setValueAt(sFile, m_Table.getSelectedRow(), m_Table.getSelectedColumn());
			m_Value = sFile;
	...
	private void assignOutput() {
		...
		fc.setPreferredSize(new Dimension(FILE_BROWSER_WITH_ACCESSORY_WIDTH, FILE_BROWSER_WITH_ACCESSORY_HEIGHT));
		
		if ( ParameterCellPanel.LAST_DIR != null ) {
			fc.setCurrentDirectory(new File(ParameterCellPanel.LAST_DIR));
		} else {
			fc.setCurrentDirectory(new File(SextanteGUI.getOutputFolder()));
		}
		...
		FILE_BROWSER_WITH_ACCESSORY_WIDTH = fc.getWidth();
		FILE_BROWSER_WITH_ACCESSORY_HEIGHT = fc.getHeight();
		...
		if (returnVal == JFileChooser.APPROVE_OPTION) {
			...
			if (m_Object instanceof OutputRasterLayer) {
				sFile = SextanteGUI.checkAddExtension(sFile, ".tif");
			}
			if (m_Object instanceof OutputVectorLayer) {
				sFile = SextanteGUI.checkAddExtension(sFile, ".shp");
			}
			...
			if (iAutoFill == AutoCompletePanel.AUTOFILL_NUMBER) {
				int iCount = 1;
				for (int i = iRow; i < m_Table.getRowCount(); i++) {
