   ...
   private boolean addResults() {
      ...
      for (int i = 0; i < m_Output.getOutputObjectsCount(); i++) {
         ...
         if (object instanceof IRasterLayer) {
            final OpenJUMPOutputFactory factory = (OpenJUMPOutputFactory) SextanteGUI.getOutputFactory();
            final WorkbenchContext context = factory.getContext();
            final IRasterLayer layer = (IRasterLayer) object;
            RasterImageLayer openJUMPLayer = (RasterImageLayer) layer.getBaseDataObject();
            if (openJUMPLayer == null) {
               openJUMPLayer = (RasterImageLayer) ((IRasterLayer) SextanteGUI.getInputFactory().openDataObjectFromFile(
                        ((FileOutputChannel) channel).getFilename())).getBaseDataObject();
            }
            context.getLayerManager().addLayerable(StandardCategoryNames.WORKING, openJUMPLayer);
            ...
            if (openJUMPLayer == null) {
               openJUMPLayer = (Layer) ((IVectorLayer) SextanteGUI.getInputFactory().openDataObjectFromFile(
                        ((FileOutputChannel) channel).getFilename())).getBaseDataObject();
            }
            context.getLayerManager().addLayer(StandardCategoryNames.WORKING, openJUMPLayer);
            SextanteGUI.getInputFactory().addDataObject(layer);
         }
