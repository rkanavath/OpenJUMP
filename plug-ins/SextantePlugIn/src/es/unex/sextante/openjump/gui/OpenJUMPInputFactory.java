...
public class OpenJUMPInputFactory extends AbstractInputFactory {
    ...
    public void createDataObjects() {
        ...
        final List<?> layerables = m_Context.getLayerManager().getLayerables(
                Layerable.class);

        for (int i = 0; i < layerables.size(); i++) {
            System.out.println("Nom de la couche: "
                    + ((Layerable) layerables.get(i)).getName());
            ...
            if (layerable instanceof Layer) {
                ...
                try {
                    ...
                    ((OpenJUMPRasterLayer) obj)
                            .create((RasterImageLayer) layerable);
                    layers.add(obj);
