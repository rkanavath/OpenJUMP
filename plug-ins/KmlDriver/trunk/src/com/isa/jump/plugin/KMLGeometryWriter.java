...
/**
 * Writes or creates a formatted string containing the KML
...
public class KMLGeometryWriter
{
  ...
  public void setParameters(String zoneStr, double centralMeridian)
  {
  	if (zoneStr.length() > 0)
  	{
  		...
  		this.zoneInt = Integer.parseInt(zoneStr.substring(0, zoneStr.length()-1));
  ...
  private void write(Coordinate[] coords, int level, StringBuffer buf) {
  	...
  	if (zoneInt > 0)
  	{
  		...
  		source = new CoordinateSystem(
  				"UTM "  + zoneStr + " / WGS 84", 32600 + zoneInt,
    ...
    for (int i = 0; i < coords.length; i++) {
      ...
      if (source != null) Reprojector.instance().reproject(coord, source, destination);
      buf.append(coord.x);
