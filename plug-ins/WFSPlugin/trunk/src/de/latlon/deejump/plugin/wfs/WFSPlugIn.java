...
public class WFSPlugIn extends ThreadedBasePlugIn {
	...
	context.getWorkbenchContext().getWorkbench().getFrame().getToolBar()
		.addPlugIn(getIcon(), this, mec, context.getWorkbenchContext());
	...
	Envelope env = context.getLayerViewPanel().getViewport()
	...
	org.deegree.model.feature.FeatureCollection dfc = JUMPFeatureFactory
		.createDeegreeFCfromWFS(rd.getWFSPanel().getWfService(),
			request, rd.getWFSPanel().getFeatureType());

	monitor
		.report("Parsing feature collection (size = " + dfc.size()
			+ ")");
	...
	if (dfc != null) {
		...
		LOG
			.warn("Could not determine the qualified name of the geometry property. Setting it to 'GEOMETRY'.");
	    }
	    ...
	    String displayName = AbstractWFSWrapper.WFS_PREFIX + ":"
		    + ftName.getLocalName();
	    WFSLayer layer = new WFSLayer(displayName, layerManager
		    .generateLayerFillColor(), dataset, layerManager, ftName,
		    geoQN, crs, rd.getWFSPanel().getWfService());
	    ...
	    /*
                 * WFSLayerListener layerListener = new WFSLayerListener(
                 * displayName ); layerManager.addLayerListener( layerListener );
                 * layer.setLayerListener( layerListener );
                 */
	    /*
                 * DataSource ds = new WFSDataSource();
                 * 
                 * Map map = new HashMap(2); map.put( "SERVER_URL", this.wfsUrl );
                 * map.put( "REQUEST", "<![CDATA["+ request +"]]>" );
                 * ds.setProperties( map );
                 * 
                 * //TODO fix this "query to come...", "dataSourceQuery"
                 * DataSourceQuery wfsDSQuery = new DataSourceQuery( ds, "query
                 * to come...", "dataSourceQuery");
                 */
	    // fix for threading problem:
	    // don't allow event fireing
	    /*
                 * Disabled due to some rather strange behaviour in OJ1.2b when
                 * adding a WFS-Layer with opened system-tab inside the
                 * layerlist.
                 * 
	    ...
	    /*
                 ...
                 * 
                 * //fire! layerManager.fireLayerChanged(layer,
		...
		if (!editingPlugIn.getToolbox(context.getWorkbenchContext())
			.isVisible()) {
	...
	PersistentBlackboardPlugIn.get(context.getWorkbenchContext()).put(
		WFSDialog.WFS_URL_LIST, StringTools.arrayToString(urls, ','));
    }
    ...
    private org.deegree.model.spatialschema.Geometry getSelectedGeoAsGML(
	    Collection geoCollec, String srs) throws Exception {
    ...
    /*
         * not used anymore public static final
         ...
         * Coordinate(env.getMinX(), env.getMinY()); coords[1] = new
         * Coordinate(env.getMinX(), env.getMaxY()); coords[2] = new
         * Coordinate(env.getMaxX(), env.getMaxY()); coords[3] = new
         * Coordinate(env.getMaxX(), env.getMinY()); coords[4] = coords[0];
         * 
         * GeometryFactory gf = new GeometryFactory(); LinearRing shell =
         * gf.createLinearRing( coords ); Polygon poly = gf.createPolygon(
         * shell, null ); GM_Object geoObj = JTSAdapter.wrap( poly );
         * 
         * GMLGeometry gg = GMLFactory.createGMLGeometry( geoObj );
         * gg.setSrs(srs); return gg; }
         */
	...
	String urls = (String) PersistentBlackboardPlugIn.get(workbenchContext)
