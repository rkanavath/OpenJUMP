...
public class UpdateWFSLayerPlugIn extends ThreadedBasePlugIn {
	...
	HashMap changedFeaturesMap = layer.getLayerListener()
		.getChangedFeaturesMap();

	ArrayList updateGeomFeatures = (ArrayList) changedFeaturesMap
		.get(UPDATE_GEOM);
	ArrayList updateAttrFeatures = (ArrayList) changedFeaturesMap
		.get(UPDATE_ATTR);
	HashMap oldGeomFeatures = layer.getLayerListener()
		.getOldGeomFeaturesMap();
	HashMap oldAttrFeatures = layer.getLayerListener()
		.getOldAttrFeaturesMap();
	...
	updateRequest = TransactionFactory.createCommonUpdateTransaction(
		context.getWorkbenchContext(), layer.getQualifiedName(),
		updateGeomRequest, updateAttrRequest);

	// DELETE
	if (delFeatures.size() > 0) {
	    deleteRequest = TransactionFactory.createTransaction(context
		    .getWorkbenchContext(), DELETE, layer.getQualifiedName(),
		    null, delFeatures);
	}

	// INSERT
	if (newFeatures.size() > 0) {
	    insertRequest = TransactionFactory.createTransaction(context
		    .getWorkbenchContext(), INSERT, layer.getQualifiedName(),
		    geoPropName, newFeatures);
	    hasInserted = true;
	}
	...
	try {
		...
		mesg.append(doTransaction("UPDATE", updateRequest.toString()));
	    }

	    if (insertRequest != null && !"".equals(insertRequest.toString())) {
		mesg.append(doTransaction("INSERT", insertRequest.toString()));
	    }

	    if (deleteRequest != null && !"".equals(deleteRequest.toString())) {
		mesg.append(doTransaction("DELETE", deleteRequest.toString()));
	    }
		...
		context.getSelectedLayer(0).fireLayerChanged(
			LayerEventType.REMOVED);
		context.getSelectedLayer(0).fireLayerChanged(
			LayerEventType.ADDED);
	    }
	...
	doc = XMLTools.parse(new InputStreamReader(httpMethod
		.getResponseBodyAsStream()));

	String partialResult = DOMPrinter.nodeToString(
		doc.getDocumentElement(), "UTF-8");
	...
	URL url = UpdateWFSLayerPlugIn.class
		.getResource("transationresp2html.xsl");

	String s = DOMPrinter
		.nodeToString(doc, CharsetUtils.getSystemCharset());
    ...
    private boolean checkGeometries(Class comparisonGeo,
	    FeatureCollection featCollec) {
    ...
    private void reloadLayer(TaskMonitor monitor, PlugInContext context,
	    String layerName) throws Exception {
	monitor.report("AddWFSQueryPlugIn.generating");

	Envelope jEnv = context.getLayerViewPanel().getViewport()
		.getEnvelopeInModelCoordinates();

	org.deegree.model.spatialschema.Envelope env = GeometryFactory
		.createEnvelope(GeometryFactory.createPosition(jEnv.getMinX(),
			jEnv.getMinY()), GeometryFactory.createPosition(jEnv
			.getMaxX(), jEnv.getMaxY()), null);

	GetFeature gfr = null;

	org.deegree.model.feature.FeatureCollection dfc = JUMPFeatureFactory
		.createDeegreeFCfromWFS(wfs, gfr);
	...
	MultiEnableCheck mec = new MultiEnableCheck()
		.add(
			createExactlyNWfsLayersMustBeSelectedCheck(
				workbenchContext, 1)).add(
			createFeatureMustHaveChangedCheck(workbenchContext));

	return mec;
    }

    public static EnableCheck createExactlyNWfsLayersMustBeSelectedCheck(
	...
	return new EnableCheck() {
		...
		n != (workbenchContext.getLayerNamePanel()).selectedNodes(
			WFSLayer.class).size())
    ...
    public EnableCheck createFeatureMustHaveChangedCheck(
	    final WorkbenchContext workbenchContext) {
	return new EnableCheck() {
	    public String check(JComponent component) {

		return (workbenchContext.getLayerNamePanel().selectedNodes(
			WFSLayer.class).size() != 1

		||

		!((Layer) workbenchContext.getLayerNamePanel().selectedNodes(
			WFSLayer.class).iterator().next())
