...
public class TransactionFactory {
     ...
     * 
     * @param geomFragment
     *                the update fragment containing features with changed
     *                geometries
     * @param attrFragment
     *                the update fragment containing features with changed
     *                attributes
    ...
    public static final StringBuffer createCommonUpdateTransaction(WorkbenchContext context,
	    QualifiedName featureType, StringBuffer geomFragment,
	...
	sb.append("xmlns:").append(
		featureType.getPrefix()).append("='").append(
		featureType.getNamespace()).append("'  >");
     ...
     *                (FeatureEventType.GEOMETRY_MODIFIED or
     ...
     * @param oldFeatures
     *                list containing original features (those are used as
     *                filter)
     * @return an XML fragment containing an update transaction
     */
    public static final StringBuffer createUpdateTransaction(
	    FeatureEventType fet, QualifiedName featureType,
	    QualifiedName geoPropName, ArrayList newFeatures,
	    HashMap oldFeatures) {
	StringBuffer sb = new StringBuffer();
	if (featureType == null) {
	    return sb;
	...
	appendUpdate(fet, sb, featureType, geoPropName, newFeatures,
		oldFeatures);

	return sb;
    }

    public static final StringBuffer createTransaction(WorkbenchContext context,
	    FeatureEventType transacType, QualifiedName featureType,
	    QualifiedName geoPropName, ArrayList newFeatures) {
    ...
    private static final void appendUpdate(FeatureEventType fet,
	    StringBuffer sb, QualifiedName featureType,
	    QualifiedName geoPropName, ArrayList features, HashMap oldFeatures) {

	for (Iterator iter = features.iterator(); iter.hasNext();) {

	    Feature feat = (Feature) iter.next();
	    sb.append("<wfs:Update typeName='").append(featureType.getPrefix())
		    .append(":").append(featureType.getLocalName())
		    .append("'>");
	    sb.append(createPropertiesFragment(geoPropName, featureType, fet,
		    feat));
     ...
     */
    private static final void appendInsert(StringBuffer sb,
	...
	for (Iterator iter = features.iterator(); iter.hasNext();) {
	    Feature feat = (Feature) iter.next();
	    String s = new StringBuilder().append(featureType.getPrefix())
		    .append(":").append(featureType.getLocalName()).toString();

	    sb.append("<").append(s).append(">");
	    sb.append(createInsertPropertiesFragment(geoPropName, featureType,
		    feat));
	    sb.append("</").append(s).append(">");
	}
     ...
     */
    private static final void appendDelete(StringBuffer sb,
	    QualifiedName featureType, ArrayList features) {
	for (Iterator iter = features.iterator(); iter.hasNext();) {
	    sb.append("<wfs:Delete typeName='").append(featureType.getPrefix())
		    .append(":").append(featureType.getLocalName())
		    .append("'>");
    ...
    /**
     * Creates a StringBuffer containing the gml representation of a
     * geometry
     * 
    ...
    private static final StringBuffer createPropertiesFragment(
	    QualifiedName geoPropName, QualifiedName featureType,
	...
	for (int j = 0; j < os.length; j++) {
		...
		if (attValue != null) {
		    sb.append("<wfs:Property><wfs:Name>").append(
			    featureType.getPrefix()).append(":")
			    .append(attName).append("</wfs:Name>").append(
				    "<wfs:Value>").append(attValue).append(
	...
	for (int j = 0; j < attributes.length; j++) {
		...
		LOG
			.debug(attName.equals("GEOMETRY") ? "Schema not loaded? Using strange mechanisms here!"
				: "Ok, using schema.");
		sb.append("<").append(featureType.getPrefix()).append(":")
			.append(
				attName.equals("GEOMETRY") ? geoAttName
					.getLocalName() : attName).append(">");
		sb.append(createGeometryGML(bf.getGeometry()));
		sb.append("</").append(featureType.getPrefix()).append(":")
			.append(
				attName.equals("GEOMETRY") ? geoAttName
    ...
    /**
     * Creates a fragment with a PropertyIsEqualTo filter operation from a
     * feature bf.
     * 
     * @param bf
     * @return
	...
	for (int j = 0; j < os.length; j++) {
		...
		if (attValue != null) {
		    ...
		    if (value == 0d) {
			String attNameWoPrefix = attName.substring(attName
				.indexOf(":") + 1, attName.length());
			QualifiedName qn = new QualifiedName(featureType
				.getPrefix(), attNameWoPrefix, featureType
				.getNamespace());
			AbstractOperation oper = new PropertyIsCOMPOperation(
				100, new PropertyName(qn), new Literal(attValue
					.toString()));
