...
public class GeoUtils
{
    ...
    {   //return Bearing in radians (-PI to +PI) from startPt to endPt
        Coordinate r = new Coordinate(endPt.x - startPt.x, endPt.y - startPt.y);
        double rMag = Math.sqrt(r.x * r.x + r.y * r.y );
        if (rMag == 0.0)
        {
    ...
    public static Coordinate getClosestPointOnSegment(Coordinate r, Coordinate p0, Coordinate p1)
    {
    	...
    	//int[] which = new int[1];
    	getDistanceSqd( r,  p0,  p1, coordOut);	//,  which);
    	return coordOut;
    }
...
//    	return i; 
    ...
    public static Coordinate intersectSegments(Coordinate p0, Coordinate p1, Coordinate p2, Coordinate p3) 
    {
        ...
        double Vx = (p1.x - p0.x); double Vy = (p1.y - p0.y);
   ...
   /**
    ...
    public static boolean isMultiGeometry(Geometry geom) {
    	return (geom instanceof Polygon)
    		...
    		//geom instanceof GeometryCollection || geom instanceof MultiPolygon
    		//	|| geom instanceof MultiLineString || geom instanceof MultiPoint;
    }
...
//    }
    	   	...
    	   	for (int j = 0; j < coordsOne.length - 1; j++)
        	{
        		...
        		for (int k = 0; k < coordsTwo.length - 1; k++)
        		{
        			...
        			if ( coord != null) {  //a segment of 1 crossed 2
        				coord1.x = coord.x; coord1.y = coord.y;  				
        				coord2.x = coord.x; coord2.y = coord.y;  //both points are the same				
    			...
    			final Coordinate coord1, final Coordinate coord2)
    {
    	...
    	for (int j = 0; j < coordsOne.length; j++) {
    		...
    		for (int k = 0; k < coordsTwo.length - 1; k++) {
    			...
    			if (dist < distSqd) {
    				distSqd = dist;
    				coord1.x = pt0.x;   coord1.y = pt0.y;   //points from geom1 go to coord1
       				coord2.x = coord.x; coord2.y = coord.y; //points from geom2 go to coord2  				
    			}
    		}
    	}
    	...
    	for (int j = 0; j < coordsTwo.length; j++) {
    		...
    		for (int k = 0; k < coordsOne.length - 1; k++) {
    			...
    			if (dist < distSqd) {
    				distSqd = dist;
    				coord1.x = coord.x; coord1.y = coord.y; //points from geom1 go to coord1			
    				coord2.x = pt0.x;   coord2.y = pt0.y;   //points from geom2 go to coord2 
    	       	}
    		}
    	}
     ...
     */
    public static Collection findMedialAxisCollection(Collection features, 
...
//  			addDebugLine(p0, p1, Integer.toString(i), Double.toString(symmetry));
    ...
    private static double measureSymmetry(Coordinate p0, Coordinate p1, Coordinate[] coords) {
    	...
    	return Math.round(symmetryTotal * 1000) / 1000; //round to nearest .001
    }
    ...
    public static Geometry makeClosedLineStringsPolygons (Geometry geometry) {
    	if (geometry instanceof LineString) {
    		...
    		if (n>2 && coords[0].equals(coords[n-1])) {
    			geometry = new GeometryFactory().createPolygon( new GeometryFactory()
