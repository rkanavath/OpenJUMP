...
public class OrthogonalizePlugIn extends AbstractPlugIn implements ThreadedPlugIn{
   ...
   public void initialize(PlugInContext context) throws Exception
    ...
    public boolean execute(final PlugInContext context) throws Exception {
        ...
        if ((layers.size() > 1) || !((Layer) layers.iterator().next()).isEditable()) {
        	newLayer = true;
     ...
     */
    private boolean orthogonalize(PlugInContext context, TaskMonitor monitor) throws Exception{
	    final Collection features = context.getLayerViewPanel().getSelectionManager().getFeaturesWithSelectedItems();
	    Collection layers = context.getWorkbenchContext().getLayerViewPanel()
	    	.getSelectionManager().getLayersWithSelectedItems();
		EditTransaction transaction = new EditTransaction(features, this.getName(), 
	    ...
	    for (Iterator iter = features.iterator(); iter.hasNext();) {
	    	...
	    	transaction.setGeometry(count, resultgeom); //commit changes to undo history
	    	count++;
    ...
    public LinearRing alignPolyRing(LineString ring) { //, int anchor)  {
    	...
    	LinearRing linearRing = new LinearRing(
    			new CoordinateArraySequence(coords),ring.getFactory());
    	return linearRing;
    }
