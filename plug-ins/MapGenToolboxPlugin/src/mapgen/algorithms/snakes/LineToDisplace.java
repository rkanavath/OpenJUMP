/*****************************************************
 * created:  		04.07.2005
 * last modified:  	21.08.2005 change in energy calculation division 
 ...
 * 
 * description:
 * 		decribes a line which is in the pool of possible lines which might be displaced.<p>
 * 		The object stores the differenz line states during displacement, the energy states
 *      and energy access/calculation methods.  
 *****************************************************/
...
/**
 * @description:
 * 		stores the differenz line states during displacement, the energy states
 *      and energy access/calculation methods  
 *  
 * @author sstein
 *
...
public class LineToDisplace {
	private Feature originalJumpFeature;
	private Geometry geometryTemp;
	private ArrayList allGeometryStates = new ArrayList();
	private ArrayList dispVectorDxMatrixList = new ArrayList();
	private ArrayList dispVectorDyMatrixList = new ArrayList();
	private ArrayList intEnergyMatrixList = new ArrayList();
	private ArrayList extEnergyMatrixList = new ArrayList();
	private Matrix extEnergy;
	private Matrix intEnergy;
	private Matrix deltaIntEnergyToOrigState;
	private ArrayList integralEnergyList = new ArrayList(); //contains overall energy during pocessing for every step
	private ArrayList integralEnergyViaVertexList = new ArrayList();
	...
	public LineToDisplace(Feature f, int dispFeatureID){
		...
		this.geometryTemp = (Geometry)this.originalJumpFeature.getGeometry().clone();
	...
	public void checkIfclosed(double crucialDistance){
		...
		if ((line.isClosed() == true)|| (dist < crucialDistance)){				
	 ...
	 */
	public ArrayList getDispVectorDyMatrixList() {
	 ...
	 */
	public ArrayList getDispVectorDxMatrixList() {
	 ...
	 * 			of network state
	 */
	public ArrayList getIntegralEnergyList() {
	 ...
	 */
	public ArrayList createJumpPointFeaturesOfTempGeom(){
	    ArrayList myFeatures = new ArrayList();
	    //-- create Schema
        ...
        if(this.geometryTemp instanceof LineString){
            ...
            for(int i=0; i < myLine.getNumPoints(); i++){        
		        ...
		        point.setAttribute("eExt",new Double(energies[i][0]));
		        point.setAttribute("eInt",new Double(energies[i][1]));
		        point.setAttribute("integralEnergy",new Double(energies[i][2]));
	...
	/**
	 * @param increases noOfdisplacements + 1 .
	 */
	 ...
	 */
	public ArrayList getAllGeometryStates() {		
		ArrayList geoms = new ArrayList();
		geoms.addAll(this.allGeometryStates);
	...
	public boolean hasConflicts(){
		...
		if (this.hasExternalConflicts()== true){
			double[][] energies = this.getVertexEnergies();
	...
	public double[] getIntegralEnergieListViaVertices(){
		...
		for (int i = 0; i < eng.length-1; i++) {
			Double val = (Double)this.integralEnergyViaVertexList.get(i);
			eng[i] = val.doubleValue();
		}		
		return eng;
	}
	...
	private void calcIntegralEnergieViaVerticesAndAddToList(){
		...
		this.integralEnergyViaVertexList.add(new Double(integralEnergy));
	}
	 ...
	 * 
	 */
	public ArrayList getExtEnergyMatrixList() {
	 ...
	 */
	public ArrayList getIntEnergyMatrixList() {
	...
	public Matrix getDeltaIntEnergyToOrigState() {
		...
		if ( size > 1){
			this.deltaIntEnergyToOrigState = SnakesEnergyDisplacement.calcIntEnergyDiff(
					(Matrix)this.getIntEnergyMatrixList().get(0),
					(Matrix)this.getIntEnergyMatrixList().get(size-1));
		}
