 ...
 * 
 * description:
 * 	tests if the distance among a polygon and a Line is under a threshold
 * 	and returns the displacement vectors (for centroid or line points),
...
public class PolygonLineDistance {
    ...
    private Polygon p1;
    private LineString lin;
    private Geometry bufferPoly = null;
    private Geometry bufferLine = null;
     ...
     * @param lineSignatureWidth radius of line signatur [in metres]
     *  
     */
    public PolygonLineDistance(Polygon poly1, LineString line, double minDist, double lineSignatureWidth){
        ...
        //else{
	        ...
	        if (bufferPoly.disjoint(this.bufferLine) == false){
    ...
    public void calcDisplacementVectors(){
        ...
        if (this.commonEdge == true){
            ...
            this.calcDispVectorForIntersection((Polygon)p1.clone(), this.lin);
        }
	...
	private void calcMeanDisplacementForPolygon(){
	    ...
	    for(int i = 0; i < this.dispVecPtEgLSthisPolyList.size(); i++){	        
	        LineString myDispV = (LineString)this.dispVecPtEgLSthisPolyList.get(i);
	    ...
	    for(int i = 0; i < this.dispVecPtEgLSOtherPolyList.size(); i++){	        
	        LineString myDispV = (LineString)this.dispVecPtEgLSOtherPolyList.get(i);
	        angle = SecondGeodeticTask2d.calcAngle2Points(myDispV.getStartPoint(),
	                											 myDispV.getEndPoint());
	...
	private void calcDispVectorForIntersection(Polygon p1, LineString line){
    	...
    	Coordinate[] closestC = DistanceOp.closestPoints(intersection,center1);	
	    ...
	    //this.minWidthDoubleList.add(new Double(distToMove));
