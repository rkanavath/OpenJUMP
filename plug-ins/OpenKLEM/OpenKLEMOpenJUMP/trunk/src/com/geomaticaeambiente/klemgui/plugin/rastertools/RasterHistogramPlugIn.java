        ...
        this.mainPanel = new MainPanel(super.getInitialDialog(), componentsWithActions, false, false, true,
            ...
            public void rightButton() {
                try{
                    ...
                    //input values
                    String inRaster = GUIUtils.getStringValue(componentsWithActions.getComponent("00", GUIUtils.INPUT, 1));

                    JComboBox jComboBox_Algo = (JComboBox) componentsWithActions.getComponent("01", GUIUtils.INPUT, 1);
                    String selectedAlgo = jComboBox_Algo.getSelectedItem().toString();
                    
                    String interval = GUIUtils.getStringValue(componentsWithActions.getComponent("02", GUIUtils.INPUT, 1));
                    String base = GUIUtils.getStringValue(componentsWithActions.getComponent("03", GUIUtils.INPUT, 1));
                    
                    checksValues(inRaster, selectedAlgo, interval, base);
                    
                    DoubleBasicGrid rasterGrid = RasterUtils.getDoubleBasicGrid(
                            (CustomComboBox.RasterComboBox) componentsWithActions.getComponent("00", GUIUtils.INPUT, 1));
                    
                    HistogramCalculator rasterHisto = new HistogramCalculator();
                    Histogram histo = null;
                    
                    if(selectedAlgo.equalsIgnoreCase(AUTO)) {
                        histo = rasterHisto.calcStatsContinuous(rasterGrid, new Autoscale(rasterGrid));
                        ...
                        double intervalVal = Double.parseDouble(interval);
                    ...
                    PersonalChartHistogram persChartHisto = new PersonalChartHistogram();
                    
                    mainTabelPane.setComponentAt(1, persChartHisto.buildHistogramPanel(
                            this.getInitialDialog().getTabbedPane(), histo));
