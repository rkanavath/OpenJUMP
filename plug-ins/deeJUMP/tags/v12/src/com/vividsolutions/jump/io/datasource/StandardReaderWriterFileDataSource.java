...
public abstract class StandardReaderWriterFileDataSource
    ...
    public static final String TEMPLATE_FILE_EXTENSION = ".xsd";
    ...
    */        
    ...
    public static class GML extends ClassicReaderWriterFileDataSource {
        ...
        public boolean isWritable() {
            ...
            if ( !getProperties().containsKey( DataSource.FILE_KEY ) ){
                return false;
            }
            
            String outFile = (String)getProperties().get( DataSource.FILE_KEY );
            /*
            System.out.println( "getProperties().containsKey(StandardReaderWriterFileDataSource." +
                getProperties().containsKey(StandardReaderWriterFileDataSource.FILE_KEY) );            
//UT it is enough, if there's a file available
            getProperties().put(
                StandardReaderWriterFileDataSource.OUTPUT_TEMPLATE_FILE_KEY,
                toTemplateFile( outFile ) );
                */
            return true;
        }        
        
        /**
         * Strips extension from gmlFilename and substitute if by ".xsd"
         * @author UT
         * @param gmlFilename
         * @return
         */
        public static String toTemplateFile( String gmlFilename ){
            int dotPosition = gmlFilename.lastIndexOf('.');

            if( dotPosition > 0 ){
                gmlFilename = gmlFilename.substring(0, dotPosition);
            }
            return gmlFilename + StandardReaderWriterFileDataSource.TEMPLATE_FILE_EXTENSION;
        }
