...
public class DeeJUMPExtension extends Extension {
    ...
    public void configure( PlugInContext context ) throws Exception {
        System.out.println( "configuring latlon" );
        //UT must add this eventually
        //new LayerAndWMSFeatureInfoTool().initialize(context);
        new DeeAddWMSQueryPlugIn().install(context);
        //TODO find the editing plug-in
        new WFSResearchPlugIn(null).install(context); 
        new UpdateWFSLayerPlugIn().install(context);
        new LayerStyle2SLDPlugIn().install(context);
        new SynchroWFSLayerPlugIn().initialize(context);
