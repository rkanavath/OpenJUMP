...
public class LayerAndWMSFeatureInfoTool extends SpecifyFeaturesTool {
    ...
    private static String createFeatureInfoRequest(WMSLayer wmsLayer,
	    LayerViewPanel panel, Point2D clickXY)
	    throws MalformedURLException, IOException {
	String url = wmsLayer.createRequest(panel).getURL().toExternalForm();
	url = url.replaceFirst("REQUEST=GetMap", "REQUEST=GetFeatureInfo");
	url += "&QUERY_LAYERS="
		+ StringTools.listToString(wmsLayer.getLayerNames(), ',');
    ...
    public Map wmsLayerToSpecifiedFeaturesMap(Iterator iter, Envelope envelope){
        ...
        for (Iterator i = iter; i.hasNext();) {
            ...
            if ( wmsLayer.isVisible() ){
		    	...
		    	try {
                    String ir = createFeatureInfoRequest(wmsLayer, getPanel(),
                                                                    sourcePoint);
		            URL u = new URL(ir);
