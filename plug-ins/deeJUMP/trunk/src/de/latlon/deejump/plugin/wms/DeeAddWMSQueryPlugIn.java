...
public class DeeAddWMSQueryPlugIn extends AbstractPlugIn {
    ...
    public boolean execute(final PlugInContext context)
        ...
        d.init(new WizardPanel[] {
                new DeeURLWizardPanel(validURLsList, lastWMSUrl, lastWMSVersion), new MapLayerWizardPanel(),
        ...
        String url = (String)d.getData(DeeURLWizardPanel.WMS_URL );
        if( !validURLsList.contains( url )){
    ...
    private Envelope transform( String sourceCS, String targetCS, Envelope inEnv){
        ...
        try {
            ...
            CoordinateSystem cs = CRSFactory.create( sourceCS );
            
            
            org.deegree.model.spatialschema.Envelope de = org.deegree.model.spatialschema.GeometryFactory.createEnvelope( 
                    inEnv.getMinX(), inEnv.getMinY(),
                    inEnv.getMaxX(), inEnv.getMaxY(), cs); 
            
            IGeoTransformer gt = new GeoTransformer(targetCS);

            de = gt.transform(de, sourceCS); 
            
            Point p1 =  (Point)JTSAdapter.export( GeometryFactory.createPoint (de.getMin().getX(),de.getMin().getY(), cs ));
            Point p2 =  (Point)JTSAdapter.export( GeometryFactory.createPoint (de.getMax().getX(),de.getMax().getY(), cs ));
	...
	private void initUrlList( WorkbenchContext workbenchContext ) {
        ...
        if ( validURLsList == null ){
    		 ...
    		 if(lastValidURLsList == null){
                ...
                validURLsList.add( "http://demo.deegree.org/deegree-wms/services");
                validURLsList.add( "http://localhost:8080/deegree2demo/ogcwebservice");
