 ...
 * 
 * description:
 *		calculates the minimal distance between two non adjacent edges
 *		of a polygon shape or LineString  
 * 		(algorithm by Mats Bader, AGENT Project)
 * preliminary condition: 
 * 		a) no self intersection of polygon / LineString
 * 		b) polygon / LineString needs more than four vertices
 * zero distances will not be detected (adjacent edges)<p>
 * 
 ...
 * @author sstein
 */
public class MinWidthParts/*extends JFrame*/{

    private double threshold = 0;
    private double nrFoundVerticies = 0;
    private double nrOfChecks = 0;
//    private ArrayList minWidthDoubleList = new ArrayList();
    private List dispVecPointEdgeLStringList = new ArrayList();
    private List dispVecPointPointLStringList = new ArrayList();    
...
//		                    dyAct = ClosestVector[0].getCoordinate().y - ClosestVector[1].getCoordinate().y;
		                    ...
		                    if(isPerpendicularPoint){
		                        this.dispVecPointEdgeLStringList.add(dispVector.clone());
		                    }
		                    else{
		                        this.dispVecPointPointLStringList.add(dispVector.clone());
		                    }
    ...
    public boolean hasConflicts() {
        for (int i=0; i < this.conflictMatrix.length; i++){
            for (int j=0; j < this.conflictMatrix.length; j++){
                if(this.conflictMatrix[i][j]== true){
