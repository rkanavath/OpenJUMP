...
public class MergeSelectedPolygonsPlugIn extends ThreadedBasePlugIn{
	...
	public void run(TaskMonitor monitor, PlugInContext context) throws Exception {  
		 ...
		 ************************/
		else if(this.mergeGeomsB == true){
			if(this.workOnLayer == true){
		 ...
		 ************************/	  
		else if(this.mergeByTypeB == true){
			features = this.srcLayer.getFeatureCollectionWrapper().getFeatures();
	...
	private JRadioButton merge2polysRB;
	private JRadioButton mergeGeomsRB;
	private JRadioButton mergeGeomsByTypeRB;
	...
	private void updateUIForMethod(){ 	
		...
		if ((dialog.getBoolean(MERGEGEOMS) == true) || 
				(dialog.getBoolean(MERGEBYTYPE) == true)){
			checkbox.setEnabled(true);	  		  			  	
			//-- make layer selector usable if all geoms should be merged  
			if (dialog.getBoolean(WORK_ON_LAYER) == false ){
				layerbox.setEnabled(false);
			}
			else{
		...
		if (dialog.getBoolean(MERGEBYTYPE) == true){
			checkbox.setEnabled(true);
