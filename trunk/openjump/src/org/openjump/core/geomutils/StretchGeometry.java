 ...
 * last modified: 	20.11.2005 [added LineString scaling]
 * 
 * author:			marius (IGN) and sstein (UniZH)
 * 
 * description:
...
public class StretchGeometry {
	...
	/**
	 *  Stretch the LineString along a direction with a given scale from a point.
	 *
	 *@param  geom the Line to stretch.
	 *@param  x0 the X coordinate of the point from where to stretch.
	 *@param  y0 the Y coordinate of the point from where to stretch.
	 *@param  angle the angle of the direction to stretch. (angle from the growing X axis)
	 *@param  scale the stretching scale.
	 */
	public static void stretchLine(LineString geom, double x0, double y0, double angle, double scale){
		//rotate the polygon
		rotateLS(geom, -1.0*angle);
		//stretch the shell of the polygon along the X axis
	...
	public static void rotateLS(LineString line, double angle){
			rotateLS(line.getCentroid().getX(),line.getCentroid().getY(), angle, line);
	}
	
	public static void rotateLS(double x0, double y0, double angle, LineString myLineString){
        
        double x1, y1, xn ,yn;
        LineString newGeom = myLineString;        
